...
尾根 PPG 心率用于产犊预测的算法说明。统计依据和评价见 `4.3.3/心率/report/心率统计规律与决策特征报告.md`。
...
```
runtime\python.exe 4.3.3\心率\scripts\s01_measure.py            # 逐次心率 + HeartRate 记录树
runtime\python.exe 4.3.3\心率\scripts\s02_decision_features.py  # Features/heart_rate/windows.csv / 逐小时评价表
python            4.3.3\心率\scripts\s03_statistics.py          # 统计、评价与图表（需 matplotlib）
```

...
def source(tmp_path, modality, data):
    ...
    p.write_text(json.dumps(dict(device='ABCDEF000001',cow_id='90001A',create_time=1788228000123,update_time=1788228001123,data=data,configs=None)),encoding='utf-8')
    return p
...
def test_temperature_intake_preserves_scalar_bytes_and_owner(tmp_path):
    ...
    before=p.read_bytes()
    row=ppg_intake.plan_temperature(p,tmp_path/'target',tmp_path/'cache','copy',lambda:False,digest=digest)
    assert Path(row['target']).as_posix().endswith('/Temp/2026-09-01/ABCDEF000001-90001-A/2026-09-01_10-00-00.json')
    ...
    doc=json.loads(before);doc['cow_id']='90002A';p.write_text(json.dumps(doc))
    with pytest.raises(ValueError):
        ppg_intake.plan_temperature(p,tmp_path/'target',tmp_path/'cache','copy',lambda:False,digest=digest)


def test_temperature_organization_executes_and_repeats_without_changing_source(tmp_path, monkeypatch):
    from cowmata_tailring.workspace.video_intake import organize
    monkeypatch.setenv('COWMATA_ACCESS_DIR',str(tmp_path/'access'))
    p=source(tmp_path,'Temp',38.52)
    before=p.read_bytes()
    result=organize(tmp_path/'farm',[dict(path=str(p),kind='imu')],category='calving',farm=str(tmp_path/'farm'),cache=tmp_path/'cache',job=tmp_path/'job')
    ...
    again=organize(tmp_path/'farm',[dict(path=str(p),kind='imu')],category='calving',farm=str(tmp_path/'farm'),cache=tmp_path/'cache',job=tmp_path/'job2')

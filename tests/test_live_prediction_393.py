...
def model(root):
    ...
    root.mkdir()
    fitted = RandomForestClassifier(n_estimators=2, max_depth=2, random_state=3).fit(
        np.zeros((4, len(FEATURES) * 2)), [0, 1, 0, 1]
    )
    payload = export_forest(
        fitted, list(FEATURES) + [f + "_missing" for f in FEATURES], np.zeros(len(FEATURES) * 2)
    )
    blob = json.dumps(payload).encode()
    (root / "forest.json").write_bytes(blob)
    doc = dict(
        schema=SCHEMA,
        complete=True,
...
def raw(path, cow="90001A", temperature=True):
    frames = b"".join(
        struct.pack("<I9h", 1010 + i * 20, 0, 0, 4096, 0, 0, 0, 1, 2, 3) for i in range(6001)
    )
    doc = dict(
        device="ABCDEF000001",
        cow_id=cow,
        ...
        create_time=1788228000000,
        update_time=1788228121010,
    )
    if temperature:
        doc["temperature"] = base64.b64encode(struct.pack("<2h", 3850, 3860)).decode()
...
def test_folder_needs_no_ledger_and_accepts_flat_json_cow_id(tmp_path):
    ...
    folder.mkdir()
    p = raw(folder / "one.json")
    before = p.read_bytes()
    result = predict_folder(
        folder, model(tmp_path / "model"), tmp_path / "out", tmp_path / "cache", tmp_path / "models"
    )
...
def test_folder_does_not_mix_unidentified_cow_baselines(tmp_path):
    ...
    rows = [
        dict(
            cow_id="",
            asset_id=str(i),
            start_epoch_ms=i * 600000,
            end_epoch_ms=(i + 1) * 600000,
            activity_index=float(i),
            temperature_c=38.0,
        )
        for i in range(8)
    ]
...
def test_folder_conflicting_cow_and_missing_ppg_timing_are_reported(tmp_path):
    ...
    folder = tmp_path / "ABCDEF000001-90002-A"
    folder.mkdir()
    raw(folder / "one.json")
...
def test_folder_temperature_only_does_not_invent_risk(tmp_path):
    ...
    folder = tmp_path / "input"
    folder.mkdir()
    (folder / "temp.json").write_text(
        json.dumps(
            dict(device="ABCDEF000001", cow_id="90001A", data=38.5, create_time=1788228000000)
        )
    )
...
def test_forecast_windows_use_actual_model_horizon_and_keep_gap_visible():
    ...
    rows = []
    for stamp, score in [(0, 0.9), (600000, 0.8), (3600000, None), (86400000, 0.7)]:
        rows.append(
            dict(
                cow_id="90001",
                device_id="ABCDEF000001",
                field_mark="A",
                asset_id=str(stamp),
                start_epoch_ms=stamp,
                end_epoch_ms=stamp + 600000,
                decision_epoch_ms=stamp + 620000,
                horizon_hours=6,
                motion_coverage=1,
                ppg_coverage=None,
                risk_score=score,
                warning_level="关注并复核" if score else "数据不足",
            )
        )
    ...
    assert coverage[0]["effective_signal_hours"] == pytest.approx(2 / 3, abs=0.001)


def test_missing_behavior_dependency_never_falls_back(tmp_path):
    from cowmata_tailring.algorithms.live_prediction import predict_folder

    directory = model(tmp_path / "model")
    manifest = directory / "decision.json"
    doc = json.loads(manifest.read_text())
    doc["behavior_models"] = [dict(version="expected", sha256="0" * 64, codes=["STANDING_UP"])]
    ...
    with pytest.raises(ValueError, match="缺少"):
        predict_folder(
            folder, directory, tmp_path / "out", tmp_path / "cache", tmp_path / "empty-models"
        )


def test_folder_entry_requires_manual_enable_and_no_training_inputs(
    ...
    monkeypatch.setattr(
        "cowmata_tailring.algorithms.decision_ui.QFileDialog.getExistingDirectory",
        lambda *a: str(tmp_path / "input"),
    )

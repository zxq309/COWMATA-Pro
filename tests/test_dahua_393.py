...
from cowmata_tailring.workspace.dahua_source import (
    TZ, packed_ms, normalize_chunks, normalize_file, DHFSReader,
)
...
@pytest.mark.parametrize('bad', [b'garbage', packet()[:-1], packet()+b'unknown',
    packet()[:-8]+b'bad!'+packet()[-4:]])
...
def test_strict_time_range_and_midnight_split(lo,hi,expected):
    from cowmata_tailring.workspace.dahua_media import time_ms,segments
...
def test_bad_range_is_rejected():
    ...
    with pytest.raises(ValueError):segments(1,2000,100,99)
...
def test_twenty_slot_planner_preserves_numbers_and_conflicts(tmp_path):
    ...
    source=tmp_path/'prepared.mp4';source.write_bytes(b'mp4 verified by preparation')
    target=tmp_path/'farm'
    prepared=dict(path=str(source),sha256=hashlib.sha256(source.read_bytes()).hexdigest(),
        start_ms=time_ms('2026-09-13 10:00:00'),duration_ms=1000,metadata={})
    row=video_row(prepared,'视角20',target,{})
    assert row['owner']=='视角20'
    assert Path(row['target']).relative_to(target).as_posix()=='Video/2026-09-13/视角20/2026-09-13_10-00-00.mp4'
    assert row['metadata']['camera']=='视角20'
    existing=Path(row['target']);existing.parent.mkdir(parents=True);existing.write_bytes(b'different')
    assert video_row(prepared,'视角20',target,{})['target'].endswith('__001.mp4')
    with pytest.raises(ValueError):video_row(prepared,'视角21',target,{})
...
def test_dahua_commit_reuses_covered_lease_and_has_separate_checkpoint(tmp_path,monkeypatch):
    ...
    job=tmp_path/'job';source=job/'records/one/prepared.mp4';source.parent.mkdir(parents=True);source.write_bytes(b'fixture commit only')
    root=tmp_path/'farm/产犊';token='a'*32
    prepared=dict(path=str(source),sha256=hashlib.sha256(source.read_bytes()).hexdigest(),start_ms=1789272000000,duration_ms=1000,metadata={})
    row=video_row(prepared,'视角20',root,{})
    plan=dict(adapter='cowmata-dahua-1',mode='import',farm='farm',id=token,target=str(root),sources=[dict(path=str(job/'records'),kind='video')],rows=[row],category='calving',start=row['record_date'],end=row['record_date'],created_at='test',fast_video=True,delete_unusable=False,scenario='mixed')

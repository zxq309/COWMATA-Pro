...
def motion_record():
    frames = b''.join(struct.pack('<I9h', tick, 0, 0, 4096, 0, 0, 0, 1, 2, 3) for tick in (10, 120010))
    return dict(device=DEVICE, cow_id='', uid=7, create_time=START, update_time=START+130000,
                version=2, imu=base64.b64encode(frames).decode(),
                temperature=base64.b64encode(struct.pack('<2h',3850,-200)).decode())

def scalar_record(value=38.5, stamp=START+30010):
    return dict(configs=None,cow_id='',create_by=None,create_time=stamp,data=value,
                device=DEVICE,log=None,uid=100,update_by=None,update_time=stamp,vbat=4.1)
...
def test_paired_export_materializes_temperature_without_changing_motion(tmp_path):
    ...
    payload=json.dumps(motion_record()).encode(); raw.write_bytes(payload)
    ...
    assert all(d['_temperature']['source_sha256']==hashlib.sha256(payload).hexdigest() for d in docs)
...
def test_external_temperature_reads_case_insensitive_folder_and_checks_cow(tmp_path):
    ...
    file.parent.mkdir(parents=True); file.write_text(json.dumps(scalar_record()))
    rows,issues=external_temperatures(tmp_path)
    assert len(rows)==1 and not issues
    assert rows[0]['value']==38.5
    doc=scalar_record();doc['cow_id']='99999';file.write_text(json.dumps(doc))
    rows,issues=external_temperatures(tmp_path)
...
def test_downloader_rejects_non_scalar_or_untyped_temperature(value):
    from cowmata_tailring.edge_download.core import validate_payload,DownloadError
...
def test_aux_temperature_model_accepts_same_scalar_record_as_downloader():
    ...
    got=decode_packet(scalar_record(),Context('10001',DEVICE,'binding',START),ProtocolConfig(),START+60000)
...
def test_aux_legacy_bucket_time_matches_annotation_acquisition_clock():
    ...
    sys.path.insert(0,str(Path(__file__).resolve().parents[1]/'assets/dataset_recipes'))
    from cowmata_temperature_aux.protocol import Context,ProtocolConfig,decode_packet
    got=decode_packet(motion_record(),Context('10001',DEVICE,'binding',START),ProtocolConfig(),START+130000)
...
def test_decision_import_rejects_temperature_unit_mismatch(tmp_path):
    ...
    blob=b'{}';(tmp_path/'forest.json').write_bytes(blob)
    manifest=dict(schema=SCHEMA,complete=True,feature_names=list(FEATURES),model_file='forest.json',
                  sha256=hashlib.sha256(blob).hexdigest(),median=[0]*len(FEATURES),
...
def test_manual_downloader_never_creates_hash_suffix_for_conflicting_second(tmp_path):
    ...
    target=Target(DEVICE,'10001','A');job=Job('http://localhost',tmp_path,'产犊',(target,),('motion',),start,start+timedelta(days=1))
    original=motion_record(); first,_=save_record(job,target,'motion',original,threading.Event())
    different=dict(original);different['imu']=base64.b64encode(bytes(44)).decode()
...
def test_dataset_exports_native_temp_without_relabeling_it_as_motion(tmp_path):
    ...
    raw.parent.mkdir(parents=True);raw.write_text(json.dumps(motion_record()))
    native.parent.mkdir(parents=True);payload=json.dumps(scalar_record(39.1,START+45000)).encode();native.write_bytes(payload)
    result=build_dataset([tmp_path/'farm'],tmp_path/'datasets',job=tmp_path/'job')
    destination=Path(result['root'])/'Temp/2026-08-17/ABCDEF123456-10001-A/2026-08-17_00-00-45.json'
...
def test_raw_scanner_excludes_lowercase_temp_from_ppg(tmp_path):
    ...
    native.parent.mkdir(parents=True);native.write_text(json.dumps(scalar_record()))
...
def test_automatic_download_keeps_only_timestamp_name_on_conflict(tmp_path):
    ...
    start=datetime.fromtimestamp(START/1000,CHINA)
    job=Job('http://localhost',tmp_path,'产犊',(Target(DEVICE,'10001','A'),),('motion',),start,start+timedelta(days=1))
    original=motion_record();original['cow_id']='10001A'
...
def test_motion_dedup_fingerprint_includes_embedded_temperature():
    ...
    a=motion_record();b=dict(a);b['temperature']=base64.b64encode(struct.pack('<2h',3900,-200)).decode()
    assert fingerprint(a,'motion') != fingerprint(b,'motion'), 'Equal IMU cannot hide changed temperature samples'
...
def test_training_rejects_temperature_contract_before_using_rows(tmp_path):
    ...
    evidence={'rows':[], 'temperature_contract':{'schema':'cowmata-temperature-1','unit':'fahrenheit'}}
    with pytest.raises(ValueError,match='温度|temperature'):
...
def test_fusion_never_uses_a_temperature_received_after_prediction_cutoff(tmp_path):
    ...
    raw.parent.mkdir(parents=True)
    doc=motion_record()
    doc['imu']=base64.b64encode(b''.join(struct.pack('<I9h',10+i*20,0,0,4096,0,0,0,1,2,3) for i in range(6001))).decode()
    raw.write_text(json.dumps(doc))
    temp=root/'Temp/2026-08-17/ABCDEF123456-10001-A/2026-08-17_00-00-45.json'
    temp.parent.mkdir(parents=True)
    obj=scalar_record(40.0,START+45000);obj['update_time']=START+600000;temp.write_text(json.dumps(obj))
    result=build_fusion(root,[],tmp_path/'out',tmp_path/'cache')
    assert result['rows']
    assert result['rows'][0]['temperature_c'] != 40.0, 'Future received measurement leaked into prediction'


def test_fusion_marks_embedded_temperature_unavailable_before_packet_receipt(tmp_path):
    from cowmata_tailring.algorithms.decision import build_fusion
    root=tmp_path/'farm';raw=root/'Motion/2026-08-17/ABCDEF123456-10001-A/2026-08-17_00-00-00.json'
    raw.parent.mkdir(parents=True)
    doc=motion_record();doc['update_time']=START+600000
    doc['imu']=base64.b64encode(b''.join(struct.pack('<I9h',10+i*20,0,0,4096,0,0,0,1,2,3) for i in range(6001))).decode()
    raw.write_text(json.dumps(doc))
    result=build_fusion(root,[],tmp_path/'out',tmp_path/'cache')
...
def test_combined_cow_code_matches_same_ear_tag_and_field_mark(tmp_path):
    ...
    doc=scalar_record();doc['cow_id']='10001A';file.write_text(json.dumps(doc))
    rows,issues=external_temperatures(tmp_path)
    assert len(rows)==1 and not issues
    doc['cow_id']='10001B';file.write_text(json.dumps(doc))
    rows,issues=external_temperatures(tmp_path)
    assert not rows and len(issues)==1
...
def test_temperature_discovery_ignores_unrelated_temp_ancestor(tmp_path):
    ...
    raw=root/'Motion/raw.json';raw.parent.mkdir(parents=True);raw.write_text('{}')
    temp=root/'temp/2026-08-17/ABCDEF123456-10001-A/2026-08-17_00-00-30.json'
    temp.parent.mkdir(parents=True);temp.write_text(json.dumps(scalar_record()))
...
def test_download_motion_name_uses_first_frame_acquisition_second(tmp_path,mode):
    ...
    obj=motion_record();obj['create_time']=START+995;obj['cow_id']='10001A'
    start=datetime.fromtimestamp(START/1000,CHINA);target=Target(DEVICE,'10001','A')
    job=Job('http://localhost',tmp_path,'产犊',(target,),('motion',),start,start+timedelta(days=1))
    if mode=='manual':file=save_record(job,target,'motion',obj,threading.Event())[0]
    elif mode=='automatic':file=_save_record(job,obj,threading.Event())[0]
    else:
        wear=SimpleNamespace(category='产犊',identity=SimpleNamespace(folder_name=DEVICE+'-10001-A'))
...
def test_download_repairs_corrupt_file_with_exact_backup_and_no_suffix(tmp_path,mode):
    ...
    from cowmata_tailring.edge_download.core import CHINA,Job,Target,save_record
    from cowmata_tailring.edge_download.automatic import _save_record
    obj=motion_record();obj['cow_id']='10001A'
    start=datetime.fromtimestamp(START/1000,CHINA);target=Target(DEVICE,'10001','A')
    job=Job('http://localhost',tmp_path,'产犊',(target,),('motion',),start,start+timedelta(days=1))
    save=lambda:save_record(job,target,'motion',obj,threading.Event()) if mode=='manual' else _save_record(job,obj,threading.Event())
    original=save()[0];original.write_bytes(b'corrupt original bytes')
    repaired=save()
    ...
    assert any(p.read_bytes()==b'corrupt original bytes' for p in (tmp_path/'.edge-download/recovery').iterdir())
...
def test_legacy_decision_export_includes_native_temp_without_duplicate_buckets(tmp_path):
    ...
    doc=motion_record();payload=json.dumps(doc).encode();digest=hashlib.sha256(payload).hexdigest()
    (tmp_path/'raw.json').write_bytes(payload)
    source=dict(asset_id=digest,path='raw.json',cow_id='10001',device_id=DEVICE,field_mark='A',identity_eligible=True,record_start_ms=START)
...
def test_legacy_temperature_binding_end_includes_first_frame_delay():
    ...
    with pytest.raises(PacketError,match='CROSSES_BINDING_END'):
        decode_packet(motion_record(),Context('10001',DEVICE,'binding',START,START+120005),ProtocolConfig(),START+130000)
...
def test_moved_project_mother_export_resolves_local_raw_before_stale_hint(tmp_path,monkeypatch):
    ...
    moved=tmp_path/'moved-project';moved.mkdir()
    destination=moved/doc['source']['path'];destination.parent.mkdir(parents=True,exist_ok=True)

...
def controller(tmp_path):
 ...
 window=QMainWindow();window.show()
 ...
 yield value
 value.pending_job=None;value.stop.set();value.timer.stop();value.close_timer.stop()
 if value.notification:value.notification.close()
 if value.dialog:value.dialog.close()
...
def test_startup_check_ignores_previous_process_throttle(controller,monkeypatch):
 ...
 monkeypatch.setattr(ui.core,'check_update',lambda *a,**kw:dict(version='3.9.3',sha256='b'*64))
...
def test_worker_requests_normal_close_only_when_job_authorized(tmp_path):
 calls=[]
 probe=tmp_path/'COWMATA-Progress.exe';probe.touch()
 root=tmp_path/'app';root.mkdir()
 def run(command,timeout=0):
  calls.append([str(x) for x in command]);return subprocess.CompletedProcess(command,0,b'',b'')
...
def native_launchers(tmp_path_factory):
 if os.name!='nt':pytest.skip('Windows launcher integration')
 base=tmp_path_factory.mktemp('native-close')
 compiler=Path(os.environ['WINDIR'])/'Microsoft.NET/Framework64/v4.0.30319/csc.exe'
 if not compiler.is_file():pytest.skip('Windows C# compiler is unavailable')
 ...
 result=subprocess.run([str(compiler),'/nologo','/target:winexe','/reference:System.Windows.Forms.dll','/reference:System.Drawing.dll','/reference:System.Web.Extensions.dll','/out:'+str(launcher),str(repo/'packaging/Launcher.cs')],**flags)
 assert result.returncode==0,result.stdout.decode('utf-8','replace')
 code=base/'Fixture.cs'
 code.write_text('''using System;using System.IO;using System.Windows.Forms;
...
''',encoding='utf-8')
 ...
 result=subprocess.run([str(compiler),'/nologo','/target:winexe','/reference:System.Windows.Forms.dll','/out:'+str(dummy),str(code)],**flags)
 assert result.returncode==0,result.stdout.decode('utf-8','replace')
...
def test_native_close_preserves_cancel_and_other_installations(tmp_path,native_launchers,cancel):
 ...
 for label in ('target','unrelated'):
  root=tmp_path/label;root.mkdir();shutil.copy2(dummy,root/'COWMATA.exe');(root/'package-manifest.json').write_text('{}')
  marker=root/'result'
  if label=='target' and cancel:marker.with_suffix('.cancel').touch()
  process=subprocess.Popen([str(root/'COWMATA.exe'),str(marker)],creationflags=subprocess.CREATE_NO_WINDOW)
  targets.append((root,marker,process))
 try:
  deadline=time.monotonic()+5
  while not all(m.with_suffix('.ready').exists() for _,m,_ in targets) and time.monotonic()<deadline:time.sleep(.02)
  assert all(m.with_suffix('.ready').exists() for _,m,_ in targets)
  root,marker,process=targets[0]
  result=subprocess.run([str(launcher),'--request-close',str(root)],capture_output=True,timeout=10,creationflags=subprocess.CREATE_NO_WINDOW)
  assert result.returncode==0
  deadline=time.monotonic()+3
  while not marker.with_suffix('.asked').exists() and time.monotonic()<deadline:time.sleep(.02)
  assert marker.with_suffix('.asked').exists()
  if cancel:
   assert process.poll() is None and not marker.with_suffix('.saved').exists()
  else:
   assert process.wait(timeout=3)==0 and marker.with_suffix('.saved').read_text()=='saved'

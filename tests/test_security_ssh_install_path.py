...
pytestmark = pytest.mark.skipif(
    os.environ.get("COWMATA_TEST_LIVE_SSH") != "1",
    reason="Explicit live-server integration opt-in required",
)
...
def test_pinned_connection_works_from_installation_paths(tmp_path, directory):
    ...
    root.mkdir()
    shutil.copyfile(
        ROOT / "cowmata_tailring/edge_download/ledger_known_hosts", root / "known_hosts"
    )
    config = json.loads((ROOT / "cowmata-security.json").read_text(encoding="utf8"))
    config["ssh"] = str(ROOT / config["ssh"])
...
def test_unknown_host_still_fails_closed_in_spaced_path(tmp_path):
    root = tmp_path / "COWMATA Pro 中文"
    root.mkdir()
    (root / "known_hosts").write_text("", encoding="ascii")
    config = json.loads((ROOT / "cowmata-security.json").read_text(encoding="utf8"))
    config["ssh"] = str(ROOT / config["ssh"])
    config["known_hosts"] = "known_hosts"

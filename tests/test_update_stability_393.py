...
from test_portable_updates_390 import bundle, product
...
def test_legacy_identity_rejects_unknown_or_incomplete_product(tmp_path, monkeypatch, fault):
    ...
    if fault == "manifest_tamper":
        with (root / "package-manifest.json").open("ab") as stream:
            stream.write(b" ")
    elif fault == "identity_mismatch":
        (root / "COWMATA.install-id").write_text("COWMATA-3.5.0")
    else:
...
def test_known_unversioned_portable_completes_setup_transaction(setup_transaction_path, monkeypatch):
    ...
    sha = package(payload, "3.9.3")
    job = bridge.prepare(
        root,
        setup,
        tmp_path / "jobs",
        dict(version="3.9.3", package_sha256=sha, unpacked_size=1000),
        False,
    )
...
def test_known_unversioned_portable_completes_zip_transaction(tmp_path, monkeypatch):
    root = legacy_product(tmp_path, monkeypatch)
    archive = bundle(tmp_path, "3.9.3")
    job_dir = tmp_path / "job"
    job_dir.mkdir()
    monkeypatch.setattr(portable_update, "forget_install_registration", lambda *_: None)
    result = worker.install(
        dict(
            root=str(root),
            setup=str(archive),
            job_dir=str(job_dir),
            update=portable_update.local_update(archive),
        ),
        runner=lambda args, timeout=0: subprocess.CompletedProcess(args, 0, b"3.9.3", b""),
        restart=False,
    )
    assert result["phase"] == "complete" and result["old_version"] == "3.4.3"
...
def test_legacy_setup_refuses_unowned_data_before_creating_job(tmp_path, monkeypatch):
    root = legacy_product(tmp_path, monkeypatch)
    (root / "my-label.json").write_text("KEEP")
    setup = tmp_path / "setup.exe"
    setup.write_bytes(b"installer")
...
def test_detached_probe_checks_requested_old_root(tmp_path):
    root = product(tmp_path / "old", "3.4.3")
    job = tmp_path / "job"
    job.mkdir()
    probe = job / "COWMATA-Progress.exe"
    probe.write_bytes(b"new launcher")
    calls = []
...
def test_first_rc_without_new_probe_is_not_started(tmp_path, monkeypatch):
    ...
    with pytest.raises(ValueError, match="process probe"):
        worker.running_check(
            root,
            tmp_path / "job",
            lambda *_args, **_kwargs: pytest.fail("Old RC would launch its UI"),
        )

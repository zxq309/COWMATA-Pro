...
from cowmata_tailring.workspace.video_filename import SIGNATURE, filename_wall
...
@pytest.mark.parametrize(
    "name",
    [
        "2026-02-30_16-05-16.mp4",
        "2026-08-18_26-05-16.mp4",
        "copy_2026-08-18_16-05-16.mp4",
        "2026-08-18_16-05-16.dav",
        "clip.mp4",
    ],
)
...
def media_info():
    return {
        ...
        "format": {"format_name": "mov,mp4,m4a,3gp,3g2,mj2", "duration": "12.6"},
    }


def test_named_mp4_never_calls_ocr_frame_decode_or_packet_scan(tmp_path, monkeypatch):
    path = tmp_path / "2026-08-18_16-05-16.mp4"
    path.write_bytes(b"fixture")

    def forbidden(*a, **k):
        raise AssertionError("expensive timing path must be skipped")
...
def test_nonstandard_name_still_uses_existing_verification(tmp_path, monkeypatch):
    ...
    with pytest.raises(FallbackReached):
        inspector(tmp_path).video(path, "b" * 64)
...
def test_identical_bytes_different_names_keep_distinct_calendar_and_camera(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(probe, "probe_media", lambda *a, **k: media_info())
    names = [
        "Video/2026-08-18/视角01/2026-08-18_16-05-16.mp4",
        "Video/2026-08-18/视角02/2026-08-18_17-05-16.mp4",
    ]
    for name in names:
        p = tmp_path / name
        p.parent.mkdir(parents=True, exist_ok=True)
        p.write_bytes(b"identical fixture bytes")
    cat = Catalog(tmp_path, stability_seconds=0)
    try:
        cat.scan()
        p = probe.SourceInspector(cat.root, cat.meta)
        for name in names:
            assert cat.index_one(name, p, eager=True)["state"] == "ready"
        rows = cat.rows(kind="video")
        assert len({r["asset_id"] for r in rows}) == 1
        assert {r["metadata"]["camera"] for r in rows} == {"视角01", "视角02"}
        assert {r["metadata"]["intervals"][0]["wall_start"] for r in rows} == {
            filename_wall(n) for n in names
        }
...
def test_named_legacy_mp4_reads_native_duration_without_ocr(tmp_path, monkeypatch):
    path = tmp_path / "2026-08-18_16-05-16.mp4"
    path.write_bytes(b"legacy PS in mp4 extension")
    info = media_info()
    info["streams"][0].pop("duration")
    info["format"] = {"format_name": "mpeg"}
    ...
    def forbidden(*a, **k):
        raise AssertionError("No OCR or frame extraction for trusted name")

    monkeypatch.setattr(probe, "TimestampOCR", forbidden)
    monkeypatch.setattr(probe, "extract_frame", forbidden)
...
def test_cached_discontinuous_timing_is_not_certified_by_filename(tmp_path):
    ...
    from cowmata_tailring.workspace.video_filename import bind_filename_location, metadata_from_name
    ...
    timeline = MediaTimelineIndex(
        ...
        0,
        40,
        (TimelineSegment(0, 1000, 0, 1000), TimelineSegment(9000, 10000, 1000, 2000)),
        (TimelineDiscontinuity(5, 1000, 9000, 8000),),
    )

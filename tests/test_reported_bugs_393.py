...
def test_open_long_record_shows_whole_timeline_and_restores_position(window, monkeypatch):
    ...
    window._motion_loaded((window.load_generation, window.current_row, motion, window.current_stamp, False))
...
def waveform(window):
    ...
    wave.set_data([PlotSeries('ax', 'AX', 'g', '#159c8d', times, np.full(len(times), 1.234))], 10000)
    wave.set_events([{'name':'test', 'color':'#159c8d'}], [{'id':1, 'li':0, 't0':3000, 't1':5000}])
...
def test_pan_over_selected_boundary_preserves_annotation(window, mode):
    ...
    modifiers = Qt.KeyboardModifier.ShiftModifier if mode == 'shift' else Qt.KeyboardModifier.NoModifier
    ...
    for kind, p, button, buttons in [
        ...
        (QEvent.Type.MouseMove, point + QPointF(80, 0), Qt.MouseButton.NoButton, Qt.MouseButton.LeftButton),
        (QEvent.Type.MouseButtonRelease, point + QPointF(80, 0), Qt.MouseButton.LeftButton, Qt.MouseButton.NoButton),
    ]:
...
LEGACY = [('STANDING','G'), ('LYING','H'), ('WALKING','I'), ('STRAINING_ONSET','J'), ('TAIL_RAISED','Q'), ('TAIL_WAGGING','W')]


@pytest.mark.parametrize('code,key', LEGACY)
def test_legacy_label_keyboard_and_button_both_record_without_changing_old_events(window, monkeypatch, code, key):
    ...
    window.show(); window.activateWindow(); window.labels.setFocus(); QTest.qWait(30)
    ...
    assert all(window.work.project.labels[d['label_index']].code == code for d in window.work.drafts)


def test_empty_candidate_window_explains_model_setup_and_refreshes_after_import(window, monkeypatch):
    ...
    try:
        ...
        assert opened == [True]
        packs.append({'version':'qa-model', 'hash':'a'*64, 'modality':'motion', 'models':[{'id':'STANDING_UP','title':'起立过程'}]})
...
def test_cancel_missing_video_directory_still_loads_raw_record(window, monkeypatch, tmp_path):
    ...
    def open_project(root, *, preferred_json, day, standalone):
        catalog = StandaloneCatalog(**standalone, day=day, meta_path=tmp_path/'single-cache', stability_seconds=0)
        try:
            catalog.scan(now=1)

...
def test_normal_directory_selection_returns_before_slow_enumeration(tmp_path, monkeypatch):
    ...
    for i in range(4):
        (view / (str(i) + ".mp4")).write_bytes(b"untouched")
    ...
    try:
        ...
        window.set_video_root(root)
        elapsed = time.monotonic() - begin
        assert elapsed < 0.15, "Directory selection blocked the GUI for " + str(elapsed)
        ticks = 0
...
def test_dahua_scan_discovers_sources_without_hashing_payloads(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        lambda *a, **k: (_ for _ in ()).throw(
            AssertionError("Full video read during directory discovery")
        ),
    )
...
def test_raw_page_uses_disk_dropdown_without_opening_picker(monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        lambda *a, **k: (_ for _ in ()).throw(AssertionError("Native folder picker opened")),
    )
    panel = DahuaPanel()
    ...
    calls = []
    panel.start = lambda action, request, job=None: calls.append((action, request))
    panel.disk_choice.setCurrentIndex(1)
    panel.disk_selected(1)
...
def test_selected_dahua_source_is_hashed_and_modified_source_rejected(tmp_path, monkeypatch):
    ...
    source = tmp_path / "one.dav"
    source.write_bytes(b"original")
    job = tmp_path / "job"
    index = dahua_tasks.scan(dict(mode="files", files=[str(source)]), job)
    ...
    row = index["rows"][0]
    prepared, _ = dahua_tasks.normalized(row, index, job, lambda: False)
    ...
    with pytest.raises(ValueError, match="重新扫描"):
        dahua_tasks.normalized(row, index, job, lambda: False)
    assert prepared.read_bytes() == b"original"

...
def test_transient_windows_replace_failure_retries_without_losing_transaction(tmp_path,monkeypatch):
    store=initialized(tmp_path);original=module.os.replace;calls=[]
    def replace(source,target):
        calls.append((source,target))
        if len(calls)==1:raise PermissionError('temporary reader')
        return original(source,target)
    monkeypatch.setattr(module.os,'replace',replace)
    with store.transaction() as rows:rows[0]['value']='after'
    assert len(calls)==2
    with store.transaction() as rows:assert rows[0]['value']=='after'
...
def test_external_edit_during_replace_retry_is_not_overwritten(tmp_path,monkeypatch):
    ...
    with pytest.raises(ValueError,match='外部修改'):
        with store.transaction() as rows:rows[0]['value']='ours'

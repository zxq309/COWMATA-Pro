...
class AdminPasswordTests(unittest.TestCase):
    def setUp(self):
        ...
        self.now = [10000.0]
        self.admin = {**new_credential("owner_qa"), "code": ""}
    ...
    def test_admin_no_code_repeatable_and_all_permissions(self):
        one = self.login()
        two = self.login(device_label="Second PC")
        self.assertEqual(one["role"], "admin")
        self.assertEqual(two["role"], "admin")
        self.assertNotEqual(one["device_secret"], two["device_secret"])
        for cap in ("accounts", "behavior", "health", "dataset"):
            self.a.authorize(one["session"], "pro", cap)
    ...
    def test_password_only_admin_requires_fixed_owner(self):
        with self.assertRaises(Denied):
            self.login(self.operator, role="admin")
        op = self.a.redeem(
        ...
        with self.assertRaises(Denied):
            self.a.credentials(op["session"])

    def test_operator_cannot_skip_authorization_code(self):
        with self.assertRaises(Denied):
            self.a.redeem(self.operator["account"], "", "pro", password=self.operator["password"])
    ...
    def test_blank_operator_code_revokes_without_breaking_admin(self):
        ...
        with self.assertRaises(Denied):
            self.a.device_login(op["device_secret"], "pro")

    def test_password_rotation_revokes_all_old_admin_devices(self):
        old = self.login()
        self.admin = {**self.admin, "password": "new-test-password"}
        self.write([self.admin, self.operator])
        with self.assertRaises(Denied):
            self.a.refresh(old["session"], "pro")
        with self.assertRaises(Denied):
            self.a.device_login(old["device_secret"], "pro")
        self.assertEqual(self.login()["role"], "admin")

    def test_deleted_and_readded_admin_allows_new_login_not_old_device(self):
        old = self.login()
        self.write([self.operator])
        with self.assertRaises(Denied):
            self.login()
        self.write([self.admin, self.operator])
        self.assertEqual(self.login()["role"], "admin")
    ...
    def test_admin_code_is_not_a_password_login_dependency(self):
        old = self.login()
        self.admin = {**self.admin, "code": new_credential("owner_qa")["code"]}
        self.write([self.admin, self.operator])
    ...
    def test_wrong_password_rate_limited_and_persisted(self):
        ...
        self.a = SimpleAuthority(self.path, clock=lambda: self.now[0])
        with self.assertRaisesRegex(Denied, "5"):
            self.login()
        self.now[0] += 301
        self.assertEqual(self.login()["role"], "admin")
    ...
    def test_client_password_only_protocol_and_resume(self):
        ...
        session = Session(transport, "pro", device_store=store)
        session.login_admin(self.admin["account"], self.admin["password"], remember_device=True)
        self.assertEqual(requests[0]["action"], "admin_login")
        self.assertNotIn("code", requests[0])
        self.assertTrue(session.allows("accounts"))
        self.assertNotIn("password", store.data)
        self.assertTrue(Session(transport, "pro", device_store=store).resume())
    ...
    def test_admin_client_rejects_operator_response(self):
        ...
        with self.assertRaises(AccessDenied):
            session.login_admin("owner_qa", "test")
    ...
    def test_state_does_not_store_plain_password(self):
        ...
        self.assertNotIn(
            self.admin["password"],
            (self.path.parent / ".system/state.csv").read_text(encoding="utf-8-sig"),
        )

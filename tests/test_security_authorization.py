...
class AuthorizationTests(unittest.TestCase):
    ...
    def test_reuse_fails_even_after_restart(self):
        code = self.code()
        self.auth.redeem("worker", code, "pro")
        other = Authority(self.path, clock=lambda: self.now[0])
        with self.assertRaises(Denied):
            other.redeem("worker", code, "pro")
    ...
    def test_no_raw_code_or_session_in_csv(self):
        code = self.code()
        token = self.auth.redeem("worker", code, "pro")["session"]
        raw = self.path.read_bytes()
        for secret in (code, token, self.password, self.admin):
            self.assertNotIn(secret.encode(), raw)

    def test_recovery_requires_password_and_admin(self):
        with self.assertRaises(Denied):
            self.auth.recover_grant("admin", "wrong", "pro")
        with self.assertRaises(Denied):
            self.auth.recover_grant("worker", self.password, "pro")
        with self.assertRaises(Denied):
            self.auth.bootstrap("intruder", self.password)

    def test_no_self_disable(self):
        with self.assertRaises(Denied):
            self.auth.set_active(self.admin, "admin", False)

    def test_failed_attempts_rate_limited(self):
        for _ in range(10):
            with self.assertRaises(Denied):
                self.auth.redeem("worker", "bad", "pro")
        with self.assertRaises(Denied):
            self.auth.redeem("worker", self.code(), "pro")
    ...
    def test_wsgi_no_cache_and_no_secret_reflection(self):
        request = json.dumps(
            {"action": "redeem", "account": "worker", "code": "a-secret", "product": "pro"}
        ).encode()
        capture = []
        body = b"".join(
            Application(self.auth)(
                {
                    "PATH_INFO": "/v1/auth",
                    "REQUEST_METHOD": "POST",
                ...
                lambda s, h: capture.append((s, dict(h))),
            )
        )
    ...
    def test_even_owner_cannot_create_another_issuer(self):
        with self.assertRaises(Denied):
            self.auth.create_user(self.admin, "another_admin", "admin")

    def test_malformed_csv_fails_closed(self):
        self.path.write_text("wrong,header\n", encoding="utf8")
        with self.assertRaises(ValueError):
            self.auth.refresh(self.admin, "pro")
    ...
    def test_deleting_grant_row_revokes_device_and_open_session(self):
        ...
        with self.auth.store.transaction() as rows:
            device = next(
                r for r in rows if r["kind"] == "device" and r["value"] == activated["device_id"]
            )
            rows[:] = [r for r in rows if not (r["kind"] == "grant" and r["id"] == device["actor"])]
        with self.assertRaises(Denied):
            self.auth.device_login(activated["device_secret"], "pro")
        with self.assertRaises(Denied):
            self.auth.refresh(activated["session"], "pro")

    def test_operator_cannot_list_or_revoke_devices(self):
        activated = self.auth.redeem("worker", self.code(), "pro")
        with self.assertRaises(Denied):
            self.auth.devices(activated["session"])
        with self.assertRaises(Denied):
            self.auth.revoke_device(activated["session"], activated["device_id"])
    ...
    def test_client_silent_login_and_expired_session_renewal(self):
        ...
        client.login("worker", self.code())
        code_count = len(
            [r for r in self.auth.store.path.read_text().splitlines() if r.startswith("grant,")]
        )
        reopened = Session(transport, "pro", device_store=saved)
        self.assertTrue(reopened.resume())
        self.now[0] += 8 * 3600 + 1
        self.assertTrue(reopened.refresh())
        self.assertTrue(reopened.allows("annotate"))
    ...
    def test_dpapi_round_trip_and_tamper_detection(self):
        ...
        value = {"device_secret": "test-sensitive-device-secret"}
        store.save(value)
        self.assertNotIn(value["device_secret"].encode(), store.path.read_bytes())
    ...
    def test_legacy_owner_hash_import(self):
        ...
        salt = b"synthetic-test-1"
        password = "previous-owner-password"
        encoded = (
        ...
        with self.auth.store.transaction() as rows:
            find(rows, "setting", "recovery")["value"] = encoded
        self.assertTrue(
            self.auth.recover_grant("admin", password, "pro")["code"].startswith("cw1_")
        )
    ...
    def test_independent_processes_redeem_once(self):
        ...
        children = [
            subprocess.Popen(
                [sys.executable, "-B", "-c", probe, module_root],
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=6) as pool:
            results = list(
                pool.map(
                    lambda child: child.communicate(
                        json.dumps([str(self.path), code]).encode(), timeout=15
                    ),
                    children,
                )
            )
    ...
    def test_client_rejects_plain_http(self):
        for endpoint in (
            ...
            with self.assertRaises(ValueError):
                HttpsTransport(endpoint)

    def test_client_no_role_escalation_from_caps(self):
        client = Session(lambda _: None, "pro")
        client.token = "t" * 43
        client._accept(
            {
                "account": "worker",
                ...
                "product": "pro",
                "lease_seconds": 900,
                "capabilities": ["accounts", "behavior", "annotate"],
            }
        )

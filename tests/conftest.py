...
def save_when_test_closes_window(monkeypatch, qt_window_registry):
    ...
    pending = [
        window
        for window in qt_window_registry.values()
    ...
    if pending:
        states = [
            {
                "closing_requested": window._closing_requested,
                ...
                if window.snapshot_writer.pending is not None
                else None,
    ...
    completed = [
        (key, window)
        for key, window in qt_window_registry.items()
...
def business_test_authority(tmp_path_factory):
    ...
    activated = authority.redeem(
        "test_admin", credential["code"], "pro", password=credential["password"]
    )

...
from .data_category import category_root,category_fields
from .dataset_access import DatasetLease,overlaps
from .dahua_source import DHFSReader,disks,normalize_file,check,TZ
from .dahua_media import probe,packet_clock,segments,time_ms,transcode,thumbnail
from .resource_layout import day_at,start_stamp,covered_days
from .storage import atomic_json,ProjectLock

ADAPTER='cowmata-dahua-1'


def task_root():
    return Path(os.environ.get('LOCALAPPDATA',str(Path.home())))/'COWMATA Annotator'/'dahua-tasks'
...
def job_file(job,name):
    ...
    if not candidate.is_relative_to(job):raise ValueError('任务文件路径越界')
    return candidate


def fresh_disk(saved):
    matches=[d for d in disks() if d['identity']==saved['identity']]
    if len(matches)!=1:raise ValueError('原磁盘未连接或磁盘身份已变化；请重新连接原盘')
    return matches[0]
...
def scan(request,job,cancelled=lambda:False,progress=lambda *_:None):
    ...
    if request['mode']=='disk':
        ...
        for row in rows:row.update(mode='disk',group='channel:'+row['channel'])
        ...
        for item in request.get('files',[]):
            ...
            if source.is_symlink():raise ValueError('原始码流来源不能是符号链接')
            if source.is_dir():
                folder_inputs.append(source)
                paths.extend(p for p in core.walk_files(source,cancelled)
                             if p.suffix.lower() in {'.dav','.dhav','.h264','.h265'})
            else:paths.append(source)
        paths=sorted(set(paths))
        if not paths:raise ValueError('没有找到 DAV/DHAV 原始码流文件')
...
def normalized(row,index,job,cancelled):
    folder=job_file(job,'records/'+row['id'][:24]);folder.mkdir(parents=True,exist_ok=True)
    ...
    if source.is_file() and saved.get('sha256')==digest_file(source,cancelled=cancelled):
        if index['mode']=='file':return source,saved
        ...
        with DHFSReader(disk['path'],disk['size'],disk['identity'],cancelled) as reader:
            ...
            if fingerprint!=row['fingerprint']:raise ValueError('原盘录像索引已变化')
        return source,saved
...
def prepare_preview(row,index,job,cancelled=lambda:False):
    ...
    if not picture.is_file():thumbnail(source,picture,cancelled,dav=True)
...
def video_row(prepared,view,root,reserved,cancelled=lambda:False):
    if view not in VIEWS:raise ValueError('归档视角必须是 01–20')
    source=Path(prepared['path']);root=Path(root)
    begin=prepared['start_ms'];end=begin+prepared['duration_ms']
    target=root/'Video'/day_at(begin)/view/(start_stamp(begin)+'.mp4')
    initial=target
    index=0
    while True:
        known=reserved.get(str(target))
        if target.exists():known=digest_file(target,cancelled=cancelled)
        if known is None or known==prepared['sha256']:break
        index+=1;target=initial.with_name(initial.stem+f'__{index:03}'+initial.suffix)
    reserved[str(target)]=prepared['sha256']
    return dict(source=str(source),target=str(target),kind='video',owner=view,
        status='existing' if target.exists() else 'ready',identity=core.identity(source),
        sha256=prepared['sha256'],size=source.stat().st_size,transfer='copy',
        record_start_ms=begin,record_end_ms=end,record_date=day_at(begin),
        covered_dates=covered_days(begin,end),timezone_offset_minutes=480,
        metadata={**prepared['metadata'],'camera':view},message='MP4 已完整解码，等待归档')
...
def select_records(index,request):
    ...
    if not rows:raise ValueError('已选映射中没有录像')
    lo,hi=time_ms(request.get('start')),time_ms(request.get('end'))
    if lo is not None and hi is not None and hi<=lo:raise ValueError('结束时间必须晚于开始时间')
    if index['mode']!='disk' or lo is None and hi is None:return rows
    ...
    for group in mapping:
        ordered=sorted([r for r in rows if r['group']==group and r['status']!='invalid'],key=lambda r:r['index_start_ms'])
        eligible=[r for r in ordered if (lo is None or r['index_end_ms']>=lo) and (hi is None or r['index_start_ms']<=hi)]
...
def prepare_record(row,index,request,job,cancelled):
    ...
    bounds=segments(started,ended,request.get('start'),request.get('end'),request.get('split_midnight',True))
    results=[]
    for lo,hi in bounds:
        options=dict(adapter=ADAPTER,source_sha256=source_info['sha256'],lo=lo,hi=hi,profile='h264-vfr-crf23-v2')
        key=hashlib.sha256(json.dumps(options,sort_keys=True).encode()).hexdigest()
        directory=source.parent/key[:16];directory.mkdir(exist_ok=True)
        ...
        metadata=dict(camera='',codec='h264',width=video['width'],height=video['height'],
            format=converted['info']['format'].get('format_name'),duration_ms=duration,
            timeline=timeline,needs_review=True,naming_only=False,time_engine=ADAPTER,
            intervals=[dict(wall_start=wall,wall_end=wall+duration,media_start=0,media_end=duration,
                            verified=False,warnings=['码流时间已恢复；相机时钟与 Motion 同步尚未人工核对'])],
            warnings=['码流时间不等于 Motion 已校准；暂停画面仍需核对'],
            dahua=dict(source_id=row['id'],original_source=row['source'],source_sha256=source_info['sha256'],
                       source_identity=row['source_identity'],index_start_ms=row.get('index_start_ms'),
                       index_end_ms=row.get('index_end_ms'),packet_start_ms=started,packet_end_ms=ended,
                       packet_clock=clock,settings=converted['settings'],full_decode_verified=True,
                       motion_calibrated=False,query_start=request.get('start'),query_end=request.get('end')))
        value=dict(path=str(target),sha256=sha,start_ms=actual_start,duration_ms=duration,metadata=metadata,options=options)
        atomic_json(directory/'prepared.json',value,backup=False)
        results.append(value)
    return results


def organize(request,job,cancelled=lambda:False,progress=lambda *_:None,on_row=lambda *_:None):
    from .resource_import import execute
    index=read_json(Path(job)/'dahua-index.json')
    if not index or index.get('adapter')!=ADAPTER:raise ValueError('请先扫描原始录像')
    selected=select_records(index,request)
    farm=Path(request['target']).resolve()
    category=request['category'];category_fields(category)
    if category=='pregnancy':raise ValueError('请选择孕早期、孕中期或孕晚期')
    ...
    if scenario=='attach_video':
        ...
        if code!=category:raise ValueError('已有 Motion 类别与所选类别不一致')
    elif scenario!='mixed':raise ValueError('未知归类方式')
    job=Path(job).resolve()
    sources=original_paths(index)+[Path(p).resolve() for p in request.get('json_sources',[])]
    ...
    plan=dict(adapter=ADAPTER,id=token,request=request,request_sha256=signature,status='preparing',rows=[])
    atomic_json(job/'dahua-plan.json',plan,backup=False)
    with ExitStack() as stack:
        lease=stack.enter_context(DatasetLease([root,job/'records',*sources],'organize',owner=token))
        if index['mode']=='disk':
            disk=fresh_disk(index['disk'])
            lockdir=task_root()/'disk-locks';lockdir.mkdir(parents=True,exist_ok=True)
            device_lock=ProjectLock(lockdir/(disk['identity']+'.lock'))
            stack.callback(device_lock.close)
            if not device_lock.acquired:raise OSError('此原盘正在被另一归类任务读取，请稍后继续')
        lease.mark_pending(token,job)
        rows=[];reserved={};issues=[];provenance=[]
        try:
            for position,row in enumerate(selected):
                check(cancelled)
                progress(position,len(selected),'读取与转换 · '+row['group'])
                try:
                    if row['status']=='invalid':raise ValueError(row.get('message','索引无效'))
                    prepared=prepare_record(row,index,request,job,cancelled)
                    for value in prepared:
                        result=video_row(value,request['mapping'][row['group']],root,reserved,cancelled)
                        rows.append(result);on_row(result)
                        provenance.append(dict(source_id=row['id'],group=row['group'],target=result['target'],
                                               sha256=result['sha256'],origin=result['metadata']['dahua']))
                except (OSError,ValueError,RuntimeError) as exc:
                    check(cancelled)
                    issue=dict(source=row['id'],original_source=row['source'],status='blocked',message=str(exc))
                    issues.append(issue);on_row(issue)
                plan.update(rows=rows,issues=issues,progress=position+1)
                atomic_json(job/'dahua-plan.json',plan,backup=False)
            if request.get('json_sources'):
                from .video_intake import plan_import
                sensor_plan=plan_import(str(farm),[dict(kind='auto',path=str(p)) for p in request['json_sources']],
                    '',None,category=category,farm=str(farm),transfer='copy',delete_unusable=False,cancelled=cancelled)
            ...
            commit=dict(adapter=ADAPTER,mode='import',schema='cowmata-resources-3.4',id=token,
                ...
                sources=[dict(path=str(job/'records'),kind='video')]+[dict(path=str(p),kind='auto') for p in request.get('json_sources',[])],
                category=category,note=request.get('note',''),created_at=core.now(),scenario=scenario,
                reference_records=reference,
                transfer='copy',delete_unusable=False,allow_partial=True,fast_video=True,
                start=dates[0],end=dates[-1],rows=rows)
            result=execute(commit,job,cancelled,progress,on_row=on_row,_lease=lease)
            for item in result.get('rows',[]):
                if item.get('status')=='blocked':
                    issue=dict(source=item.get('source',''),status='blocked',message=item.get('message',''))
                    if issue not in issues:issues.append(issue)
            attachments=root/'归类附属文件'/'大华导入'/token
            attachments.mkdir(parents=True,exist_ok=True)
            atomic_json(attachments/'来源与映射.json',dict(adapter=ADAPTER,request=request,
                slots=[dict(view=v,groups=[k for k,val in request['mapping'].items() if val==v]) for v in VIEWS],
                source_disk=index.get('disk'),records=provenance,issues=issues),backup=False)

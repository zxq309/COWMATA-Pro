...
def disk_info(number):
    ...
    try:
        ...
        def ioctl(code, source=b"", size=4096):
            ...
            if not k.DeviceIoControl(
                handle, code, inp, len(source), output, size, c.byref(count), None
            ):
...
def disk_letters():
    ...
    previous = w.DWORD()
    changed = k.SetThreadErrorMode(0x8001, c.byref(previous))
    result = {}
    try:
        ...
        for i in range(26):
            ...
            try:
                ...
                used = w.DWORD()
                if k.DeviceIoControl(
                    handle, 0x560000, None, 0, buffer, len(buffer), c.byref(used), None
                ):
        ...
        if changed:
            k.SetThreadErrorMode(previous, None)
    return result
...
def disks():
    ...
    for number in range(32):
        try:
            row = disk_info(number)
            row["letters"] = letters.get(number, [])
            result.append(row)
        except OSError:
            continue
    return result
    ...
    def __init__(self, source, size, identity, cancelled=lambda: False, *, lazy_descriptors=False):
        ...
        try:
            ...
            for index in range(64):
                ...
                if (
                    block not in (512, 4096)
                    or fragment != 2097152
                    or not 0 < count <= 8_000_000
                    or desc + count * 32 > self.size
                    or video + count * fragment > self.size
                    or desc < base
                ):
    ...
    def chain(self, part, head):
        ...
        for position in range(count):
            ...
            if position and (
                desc[0] != 2
                or u32(desc, 24) != head
                or u32(desc, 20) != previous
                or int.from_bytes(desc[2:4], "little") != position
                or desc[1] != first[1]
            ):
        ...
        for part in self.partitions:
            ...
            for index in range(part["count"]):
                ...
                row["id"] = hashlib.sha256(
                    f"{self.identity}:{part['index']}:{index}".encode()
                ).hexdigest()
                try:
                    ...
                    fingerprint = hashlib.sha256(
                        b"".join(self.descriptor(part, i) for i in chain)
                    ).hexdigest()
                    row.update(
                        chain=chain,
                        last_size=last_size,
                        index_start_ms=start,
                        index_end_ms=end,
...
def normalize_chunks(chunks, destination, cancelled=lambda: False):
    ...
    return dict(
        ...
        start_ms=start,
        end_ms=end,
        internal_channels=sorted(channels),

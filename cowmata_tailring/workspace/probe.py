...
def extract_frame(
    path: Path,
    media_ms: float = 0,
    timeline: MediaTimelineIndex | None = None,
    *,
    preroll=15.0,
    cancelled=None,
    preview_width=None,
    ...
    if timeline and timeline.native:
        ...
        if (
            not 0 <= start_byte < end_byte <= stat.st_size
            or end_byte - start_byte > 64 * 1024 * 1024
        ):
        ...
        match = re.search(
            r"\bn:\s*0\b.*?pts_time:([\d.eE+\-]+)", process.stderr.decode("utf-8", "replace")
        )
        if process.returncode or not process.stdout or not match:
            raise ValueError(
                "原生字节定位未取得可验证画面：" + process.stderr.decode("utf-8", "replace")[-300:]
            )
    ...
    if process.returncode or not process.stdout:
        raise ValueError(
            "无法解码所选位置的真实画面：" + process.stderr.decode("utf-8", "replace")[-400:]
        )
...
def build_observed_intervals(
    samples: list[dict], timeline: MediaTimelineIndex, *, tolerance_ms=2000
):
    ...
    for sample in ordered:
        if (
            samples
            and sample.get("wall_ms") is not None
            and sample.get("wall_ms") == samples[-1].get("wall_ms")
            and sample["media_ms"] - samples[-1]["media_ms"] < 1000
        ):
            continue
    ...
    for left, right in zip(samples, samples[1:]):
        ...
        intervals.append(
            {
                "wall_start": left["wall_ms"],
                "wall_end": right["wall_ms"],
                "media_start": left["media_ms"],
                "media_end": right["media_ms"],
                "verified": True,
                "warnings": [],
            }
        )
    ...
    for left, right in zip(good, good[1:]):
        if any(
            left["media_ms"] < s["media_ms"] < right["media_ms"] and s.get("wall_ms") is None
            for s in samples
        ):
            dm, dw = right["media_ms"] - left["media_ms"], right["wall_ms"] - left["wall_ms"]
            same_segment = (
                timeline.public_to_raw_ms(left["media_ms"])[1]
                == timeline.public_to_raw_ms(right["media_ms"])[1]
            )
                ...
                intervals.append(
                    {
                        ...
                        "media_end": right["media_ms"],
                        "verified": False,
                        "warnings": ["中间 OSD 未读清，当前仅为候选时间映射"],
                    }
                )
    ...
    if samples and samples[-1].get("wall_ms") is not None and intervals:
        ...
        if abs(intervals[-1]["media_end"] - last["media_ms"]) < 1:
            ...
            intervals.append(
                {
                    "wall_start": last["wall_ms"],
                    "wall_end": last["wall_ms"] + frame_ms,
                    "media_start": last["media_ms"],
                    "media_end": last["media_ms"] + frame_ms,
                    "verified": True,
                    "warnings": [],
                }
            )
    ...
    for segment in timeline.segments:
        points = [
            s for s in good if segment.public_start_ms <= s["media_ms"] < segment.public_end_ms
        ]
        if len(points) < 2 or segment.duration_ms <= 0:
            continue
        for first, second, edge in (
            (points[0], points[1], segment.public_start_ms),
            ...
            if (
                not 1 < distance <= 301000
                or delta <= 0
                or span <= 0
                or abs(delta - span) > tolerance_ms
            ):
                continue
            if any(
                i["media_start"] <= min(edge, anchor["media_ms"]) + 0.01
                and i["media_end"] >= max(edge, anchor["media_ms"]) - 0.01
                for i in intervals
            ):
                continue
...
class SourceInspector:
    def __init__(self, root: Path, meta: Path, stop: threading.Event | None = None, progress=None):
        ...
        self.timezone_minutes = int(
            read_json(self.meta / "project.json", {}).get("timezone_offset_minutes", 480)
        )
    ...
    def resource_records(self):
        if self._resource_records is None:
            self._resource_records = {
                r["path"]: r for r in read_json(self.root / "资源索引.json", {}).get("records", [])
            }
    ...
    def relative_path(self, path):
        try:
            ...
            return (
                Path("external")
                / hashlib.sha256(str(path.parent).encode()).hexdigest()[:12]
                / path.name
            )
    ...
    def video_hint(self, path: Path) -> dict:
        ...
        if start is not None:
            return {
                ...
                "end_ms": None,
                "hint_only": True,
                "basis": "classified_filename",
            }
        resource = self.resource_records.get(self.relative_path(path).as_posix())
        if resource:
            offset = resource.get("timezone_offset_minutes", 480) * 60000
            return {
                "start_ms": resource["record_start_ms"] + offset,
                "end_ms": None
                if resource.get("metadata", {}).get("naming_only")
                else resource["record_end_ms"] + offset,
                "hint_only": True,
            }
        ...
        for target in (0, 1200, 2500, 5000, 10000):
            ...
            try:
                ...
                continue
            report = self.ocr.routing_read(
                frame, filename=f"{path.name}@hint", hint=roi, raw_only=True
            )
        ...
        if (
            len(points) < 2
            or abs((points[-1][1] - points[0][1]) - (points[-1][0] - points[0][0])) > 2000
        ):
    ...
    def __call__(self, path: Path, kind: str, asset_id: str) -> dict:
        ...
        if kind == "imu":
            ...
            return {
                ...
                "samples": len(motion.times_ms),
                "create_time_ms": motion.create_time_ms,
                "update_time_ms": motion.update_time_ms,
                "version": motion.version,
                "warnings": motion.warnings,
                "time_semantics": "device_acquisition_start",
    ...
    def native_video(self, path, asset_id, native, video, info, roi=None):
        ...
        if not deferred and hasattr(self.ocr, "engine"):
            self.ocr.engine.cancelled = self.stop.is_set
        profile_key = (
            f"native|{self.relative_path(path).parent}|{native['family']}|{video.get('width')}"
        )
        samples, warnings = [], []
        saved_roi = (
            roi
            or self.roi_hints.get(profile_key)
            or ((0.4, 0.82, 1, 1) if native["family"].startswith("shenmo") else (0, 0, 0.8, 0.18))
        )
        ...
        targets = tuple(
            min(t, max(0, duration - native["frame_ms"] * 2)) for t in (0, 1000, 2500, 4000)
        )
        ...
        for target in () if deferred else targets:
            ...
            try:
                ...
                if hasattr(self.ocr, "engine"):
                    self.ocr.engine.cancelled = lambda: (
                        self.stop.is_set() or time.monotonic() > deadline
                    )
                report = self.ocr.native_check(
                    frame,
                    filename=path.name,
                    hint=saved_roi,
                ...
                samples.append(
                    {"media_ms": actual, "wall_ms": report.get("wall_ms"), "ocr": report}
                )
                if target == 0:
                    frame.save(frame_dir / f"{asset_id}.jpg", quality=90)
        ...
        if not deferred and not good and native["family"] == "hikvision-hk1":
            ...
            for attempt, fallback_target in enumerate((0, 1200, 2500)):
                ...
                target = (
                    min(
                        duration - native["frame_ms"] * 2,
                        good[-1]["media_ms"] + (attempt + 1) * 2000,
                    )
                    if good
                ...
                try:
                    frame, actual = extract_frame(
                        path, target, timeline, cancelled=self.stop.is_set
                    )
                    profile = "hd_no_weekday" if frame.width == 2560 else "sd_with_weekday"
                    report = self._recognize_bounded(
                        frame, time.monotonic() + 45, filename=path.name, profile_hint=profile
                    )
        ...
        for sample in list(good):
            ...
            positions = [
                sample["media_ms"] + 1200,
                (matched["media_ms"] + 2000) if matched else duration * 0.1,
            ]
        ...
        corner = "unknown"
        if saved_roi:
            corner = ("top" if (saved_roi[1] + saved_roi[3]) / 2 < 0.5 else "bottom") + (
                "_left" if (saved_roi[0] + saved_roi[2]) / 2 < 0.5 else "_right"
            )
        camera = folder
        return {
            "camera": camera,
            ...
            "roi": saved_roi,
            "warnings": warnings,
            "ocr_engine": self.ocr.signature if self.ocr else TIMESTAMP_SIGNATURE,
            "native_check_pending": deferred,
            "time_engine": NATIVE_SIGNATURE,
            "time_basis": native["family"],
    ...
    def video(self, path: Path, asset_id: str, *, roi=None) -> dict:
        ...
        if filename_wall(path) is not None and not roi and not correction.get("readings"):
            ...
            if native:
                ...
                timeline = probe_media_timeline(
                    path, ffprobe, timeout_seconds=180, cancelled=self.stop.is_set
                )
        ...
        if (
            self.opening_cache
            and self.opening_cache["path"] == path
            and self.opening_cache["stamp"] == file_stamp(path)
            and not roi
        ):
            opening_frames = {
                t: value
                for t, value in self.opening_cache["frames"].items()
        ...
        for segment in timeline.segments:
            ...
            targets.add(
                max(
                    segment.public_start_ms,
                    segment.public_end_ms - max(2, timeline.frame_duration_ms * 1.5),
                )
            )
        ...
        saved_roi = roi
        hint_key = "|".join(
            map(str, (self.relative_path(path).parts[0], video.get("width"), video.get("height")))
        )
        ...
        for number, target in enumerate(sorted(targets)):
            ...
            if time.monotonic() >= ocr_deadline:
                samples.append(
                    {"media_ms": target, "wall_ms": None, "ocr": self._ocr_limit_report()}
                )
                continue
            ...
            try:
                ...
                if cached:
                    ...
                    if (
                        report.get("routing_only")
                        and not report.get("success")
                        and len(opening_hits) >= 2
                    ):
                    ...
                    if report.get("routing_only"):
                        report = self._recognize_bounded(
                            frame,
                            frame_deadline,
                            filename=f"{path.name}@{actual:.0f}ms",
                            roi=roi,
                            hint=report.get("roi") or saved_roi,
                            profile_hint=layout,
                        )
                else:
                    frame, actual = extract_frame(
                    ...
                    report = self._recognize_bounded(
                        ...
                        filename=f"{path.name}@{actual:.0f}ms",
                        roi=roi,
                        hint=saved_roi or self.roi_hints.get(hint_key),
                        profile_hint=layout,
                    )
                if number == 0:
                    frame.save(frame_dir / f"{asset_id}.jpg", quality=90)
                ...
                if not report["success"] and not report.get("budget_exhausted"):
                    ...
                    for offset in (500, -500, 1500, -1500, -2500, -4000):
                        ...
                        if not 0 <= retry_at < timeline.duration_ms - timeline.frame_duration_ms:
                            continue
                        retry_frame, retry_actual = extract_frame(
                            path, retry_at, timeline, cancelled=self.stop.is_set
                        )
                        self.progress(f"复核画面时间：{path.name} @{retry_at / 1000:.1f}s")
                        retry_report = self._recognize_bounded(
                            retry_frame,
                            frame_deadline,
                            filename=f"{path.name}@{retry_actual:.0f}ms",
                            roi=roi,
                            hint=saved_roi,
                            profile_hint=layout,
                        )
            ...
            if consensus >= max(3, len(offsets) - 2):
                ...
                for sample in suspects[:2]:
                    for offset in (500, -500, 1500):
                        ...
                        report = self._recognize_bounded(
                            frame,
                            min(ocr_deadline, time.monotonic() + 12),
                            filename=f"{path.name}@{actual:.0f}ms",
                            roi=roi,
                            hint=saved_roi,
                            profile_hint=layout,
                        )
                        sample.setdefault("temporal_rechecks", []).append(
                            {"media_ms": actual, "ocr": report}
                        )
                        if (
                            report["wall_ms"] is not None
                            and abs(report["wall_ms"] - actual - centre) <= 2000
                        ):
                            sample["original_observation"] = {
                                "media_ms": sample["media_ms"],
        ...
        if any(s.get("ocr", {}).get("budget_exhausted") for s in samples):
            warnings.append(
                "部分画面时间限时未读清；已保留待复核，继续检索其他录像。可框选时间戳或输入人工读数。"
            )
        if saved_roi:
        ...
        return {
            ...
            "time_engine": NATIVE_SIGNATURE,
            "preview": f"previews/{asset_id}.jpg",
            "needs_review": bool(warnings) or not intervals,

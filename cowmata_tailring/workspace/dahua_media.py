...
def probe(path,cancelled=lambda:False,*,dav=False):
    ...
    result=run([ffprobe,'-v','error',*args,'-show_streams','-show_format','-of','json',path],cancelled,120)
...
def packet_clock(path,cancelled=lambda:False,*,dav=False):
    _,ffprobe=find_ffmpeg()
    args=['-f','dhav'] if dav else []
    value=run([ffprobe,'-v','error',*args,'-select_streams','v:0','-show_packets',
        '-show_entries','packet=pts_time,duration_time','-of','compact=p=0:nk=0',path],cancelled,300)
    ...
    for line in value.stdout.decode('utf-8','replace').splitlines():
        ...
        try:pts=float(fields['pts_time'])
        except (ValueError,KeyError):
            raise ValueError('码流缺少逐帧 PTS，不能根据猜测帧率归档') from None
        if not math.isfinite(pts):raise ValueError('码流 PTS 无效')
        ...
        count+=1
        try:duration=float(fields.get('duration_time',0))
        except ValueError:duration=0
    if count<2 or maximum<=first:
        raise ValueError('视频帧不足或 PTS 不递增')
    if max_back>.25 or max_gap>5:
        raise ValueError(f'视频时钟不连续（回退 {max_back:.3f}s / 间隔 {max_gap:.3f}s），需单独复核')
    return dict(first=first,last=maximum,duration=maximum-first+max(duration,0),
                packets=count,duplicates=duplicates,backwards=backwards,
                max_backwards_seconds=max_back,max_gap_seconds=max_gap)


def time_ms(value):
    if value in (None,''):return None
    if isinstance(value,(int,float)):
        if not math.isfinite(value):raise ValueError('时间无效')
        return int(value)
...
def transcode(source,target,offset_ms,duration_ms,cancelled=lambda:False):
    ...
    args=[ffmpeg,'-nostdin','-hide_banner','-v','warning','-xerror','-err_detect','explode',
        '-threads','2','-copyts','-start_at_zero','-f','dhav','-i',source,
        '-ss',f'{offset_ms/1000:.6f}','-t',f'{duration_ms/1000:.6f}',
        '-map','0:v:0','-map','0:a:0?','-map_metadata','-1',
        '-vf','scale=in_range=auto:out_range=tv','-c:v','libx264','-preset','veryfast',
        '-crf','23','-pix_fmt','yuv420p','-threads','2','-fps_mode','vfr',
        '-enc_time_base','1:1000','-c:a','aac','-ar','48000','-b:a','96k',
        '-movflags','+faststart','-n',target]
    ...
    run([ffmpeg,'-nostdin','-v','error','-xerror','-err_detect','explode',
         '-threads','2','-i',target,'-map','0:v:0','-map','0:a:0?',
         '-f','null','-'],cancelled,max(300,duration_ms/1000*5))
    _,ffprobe=find_ffmpeg()
    ...
    return dict(info=info,timeline=timeline.to_dict(),duration_ms=timeline.duration_ms,
                log=result.stderr.decode('utf-8','replace')[-4000:],
                settings=dict(video='h264',pixel_format='yuv420p',audio='aac',
                              pts='vfr',crf=23,original_resolution=True))


def thumbnail(source,target,cancelled=lambda:False,*,dav=False):
    ffmpeg,_=find_ffmpeg()
    args=['-f','dhav'] if dav else []
    run([ffmpeg,'-nostdin','-v','error','-threads','1',*args,'-i',source,
         '-frames:v','1','-vf','scale=320:-2','-n',target],cancelled,90)
    return str(target)

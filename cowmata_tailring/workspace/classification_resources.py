...
def limit_worker(budget=None):
    ...
    if kernel.GetProcessAffinityMask(process, ctypes.byref(current), ctypes.byref(system)):
        bits = [1 << i for i in range(ctypes.sizeof(ctypes.c_size_t)*8) if current.value & (1 << i)]
    ...
    if job:
        ...
        configured = kernel.SetInformationJobObject(job, 15, ctypes.byref(rate), ctypes.sizeof(rate))

...
def main():
    job=Path(sys.argv[1]).resolve();request=tasks.read_json(job/'dahua-request.json')
    cancelled=lambda:(job/'dahua-cancel').exists()
    last=[0.0]
    def progress(current,total,message):
        now=time.monotonic()
        if now-last[0]>.5 or current==total:
            emit(dict(event='progress',current=current,total=total,message=message));last[0]=now
    try:
        ...
        with admission:
            if action=='disks':result=dict(disks=tasks.disks())
                ...
                for position,group in enumerate(request['groups']):
                    ...
                    rows=[r for r in index['rows'] if r['group']==group and r['status']!='invalid']
                    if not rows:continue
                    try:
                        preview=tasks.prepare_preview(rows[0],index,job,cancelled)
                        row=dict(group=group,preview=preview)
                    except (OSError,ValueError,RuntimeError) as exc:
                        tasks.check(cancelled);row=dict(group=group,error=str(exc))
                    result['previews'].append(row);emit(dict(event='preview',row=row))
                    progress(position+1,len(request['groups']),'读取静态缩略图')
            elif action=='organize':
                result=tasks.organize(request['options'],job,cancelled,progress,
                    on_row=lambda row:emit(dict(event='row',row={k:v for k,v in row.items() if k in {'source','target','status','message'}})))
            else:raise ValueError('未知视频准备操作')
        atomic_json(job/'dahua-result.json',result,backup=False)
        emit(dict(event='result',path=str(job/'dahua-result.json')));return 0
    except Exception as exc:
        result=dict(error=str(exc),paused=isinstance(exc,InterruptedError))
        atomic_json(job/'dahua-error.json',result,backup=False);emit(dict(event='error',**result));return 2

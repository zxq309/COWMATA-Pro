...
def scan_dataset(root, *, pool_general_events=False, progress=lambda *_: None, cancelled=lambda: False):
    ...
    for index, label in enumerate(paths):
        ...
        try:
            ...
            asset = data.get('raw_sha256') or work.get('asset_id') or doc.get('source', {}).get('asset_id')
            ...
            for e in project.get('events', []):
                ...
                row['events'].setdefault(key, dict(code=code, start_ms=start, end_ms=end,
                    event_id=e.get('id'), confirmation=e.get('confirmation', ''), label_source=str(label)))
            # A per-event reviewed_range does NOT establish reviewed background.
            for interval in doc.get('review_coverage', []):
                if isinstance(interval, list) and len(interval) == 2 and 0 <= interval[0] < interval[1]:
                    row['review_coverage'].append(interval)
            entries.append([str(label.relative_to(root)), hashlib.sha256(content).hexdigest(), str(raw.relative_to(root)), stamp(raw)])
    ...
    for row in records.values():
        if row['conflicts']:
            issues.extend(dict(path=row['raw'], reason=reason) for reason in sorted(set(row['conflicts'])))
            if not pool_general_events or 'conflicting_device_identity' in row['conflicts']:
                continue
            row['identity_eligible'] = False
            row['events'] = {k: e for k, e in row['events'].items()
                             if e['code'] not in INDIVIDUAL_CODES}
            if not row['events']:
                continue

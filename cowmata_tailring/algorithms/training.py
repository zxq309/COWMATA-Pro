...
from . import EVENT_CODES, EVENT_TITLES, POINT_EVENT_CODES, canonical_event_code, validation_unit
...
TRAINING_VERSION = "partial-label-record-pool-2"
...
def training_rows(records, features, code):
    ...
    for record, f in zip(records, features):
        ...
        ww.extend(
            [1 / max(1, len(pos))] * len(pos) + [0.25 / max(1, len(negative))] * len(negative)
        )
    if not xx:
    ...
    weights[y == 0] *= 0.35 / weights[y == 0].sum()
    return x, y, weights
...
def _truth(record, feature, code, *, observable_only=False):
    truth = [
        ...
        if canonical_event_code(e["code"]) == code and eligible_event(e)
    ]
...
def train_suite(
    ...
    *,
    ...
    original = [
        r for r in index["records"] if r["events"] and r.get("modality", "motion") == modality
    ]
    ...
    for code in codes:
        selected = [
            ...
            if validation_unit(code) != "cow" or r.get("identity_eligible", True)
        ]
        ...
        for fold, (train, test) in enumerate(
            ...
            inner_train, inner_test = next(
                GroupShuffleSplit(n_splits=1, test_size=0.25, random_state=38 + fold).split(
                    train_r, groups=inner_groups
                )
            )
            try:
                inner_model, inner_median, _ = _fit(
                    [train_r[i] for i in inner_train], [train_f[i] for i in inner_train], code
                )
                threshold, _ = _threshold(
                    inner_model,
                    inner_median,
                    [train_r[i] for i in inner_test],
                    [train_f[i] for i in inner_test],
                    code,
                )
            except ValueError:
                threshold = 0.55
            ...
            for i in test:
                ...
                result = evaluate_events(
                    _truth(record, feature, code),
                    predictions,
                    observed_seconds=feature["observed_seconds"],
                    review_coverage=record.get("review_coverage", []),
                )
                result.pop("pairs")
                row = dict(
                    code=code,
                    ...
                    group=str(groups[i]),
                    validation_unit=validation_unit(code),
                    threshold=threshold,
                    **result,
                )
                local.append(row)
                prediction_rows.extend(
                    dict(asset_id=record["asset_id"], fold=fold, source=record["raw"], **p)
        ...
        known, hit, candidates = (
            sum(r[k] for r in local) for k in ("known_events", "matched_events", "candidates")
        )
        ...
        summary = dict(
            ...
            known_events=known,
            matched_events=hit,
            known_recall=hit / known if known else None,
        ...
        check = ff[0]["X"][:200]
        assert np.allclose(
            predict_forest(payload, check),
            model.predict_proba(np.where(np.isfinite(check), check, median))[:, 1],
            atol=1e-6,
        )
        payload.update(
            code=code,
            ...
            feature_version=ff[0]["feature_version"],
            modality=modality,
            training_version=TRAINING_VERSION,
            dataset_fingerprint=index["fingerprint"],
            score_is_probability=False,
            background_policy="unreviewed_weak_background_weight_0.35",
            fit_stats=fit_stats,
            experimental=code in POINT_EVENT_CODES,
    ...
    manifest = dict(
        shared_label_contract=dict(SHARED_LABEL_CONTRACT),

...
from .train import HOUR_MS, MANIFEST_SCHEMA, alert_episodes

RESULT_SCHEMA = "cowmata-decision-result-4.3.3"
...
def read_model(folder):
    ...
    if manifest.get("schema") != MANIFEST_SCHEMA or not manifest.get("complete"):
        raise ValueError("决策模型不完整或不是 4.3.3 决策模型")
    docs = {}
...
def _level(manifest, risk, row, low_data):
    ...
    if risk[short] >= max(float(thr.get(str(short), 0.5)), 0.5):
        return "临产", levels["临产"]
    if risk.get(primary, 0) >= float(thr.get(str(primary), 0.5)):
        return "高度关注", levels["高度关注"]
    if risk.get(primary, 0) >= 0.5 * float(thr.get(str(primary), 0.5)) or (
            horizons[-1] in risk and risk[horizons[-1]] >= float(thr.get(str(horizons[-1]), 0.5))):
        return "关注", levels["关注"]
    return "正常", levels["正常"]

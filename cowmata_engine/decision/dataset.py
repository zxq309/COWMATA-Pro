...
def attach_truth(rows, calvings, *, horizons=HORIZONS, lookback_days=10):
    from .labels import label_row_detail

    for row in rows:
        found = label_row_detail(calvings.get(row["cow_id"]), row["decision_epoch_ms"], lookback_days=lookback_days)
        if found is None:
            row.update(hours_to_calving=None, calving_epoch_ms=None, calving_start_epoch_ms=None,

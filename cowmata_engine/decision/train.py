...
def threshold_policy(thresholds, horizon):
    """Derive warning bands from held-out thresholds, without a global alarm floor."""
    primary = float(thresholds[str(horizon)])
    short = min(float(value) for value in thresholds.values())
    return dict(schema="dynamic-threshold-v1", primary_horizon=int(horizon),
                attention_factor=0.60, critical_factor=1.25,
                primary_threshold=primary, short_threshold=short,
                min_signal_coverage=0.50)


def dynamic_levels(thresholds, horizon):
    policy = threshold_policy(thresholds, horizon)
    base = policy["primary_threshold"]
    critical = min(0.999, max(policy["short_threshold"], base * policy["critical_factor"]))
    attention = max(0.001, base * policy["attention_factor"])
    values = (critical, base, attention, 0.0)
    texts = tuple((level, advice) for _, level, advice in LEVELS)
    return [dict(min_probability=float(value), level=level, advice=advice)
            for value, (level, advice) in zip(values, texts)]
...
def train_decision(dataset, output, *, algorithms=DEFAULT_ALGORITHMS, horizon=24, horizons=HORIZONS,
    ...
    if summary.get("gold_label_rows", 0):
        labelled = [r for r in labelled if r.get("training_eligible") is not False]
    if not labelled:
    ...
    manifest = dict(
        ...
        threshold_policy=threshold_policy(thresholds, horizon),

## 4.3.3 · 2026-09-26 · 7 项特征综合决策与预测

### 独立算法引擎 `cowmata_engine`（行为识别 + 综合决策，供外部前端对接）
- 新增无 Qt 依赖的算法包 `cowmata_engine`：`api.handle()` 统一 JSON 进出；`python -m cowmata_engine run/serve/info` 提供命令行与本机 HTTP 服务（`/api/<action>`，支持后台任务、进度与取消）。桌面窗口也只通过它调用算法。接口文档 `docs/ENGINE_API_433.md`。
- 行为识别门面 `cowmata_engine.behavior`（catalog / predict / train）封装起立、卧倒、努责、排尿、胎儿首见、犊牛娩出 6 类算法；候选始终只进入人工复核。
- 决策特征插件接口 `cowmata-decision-feature-1`：每项特征一个独立模块，按绝对对齐 10 min 窗口输出，`available_epoch_ms` 保证因果，缺失为空、不用 0 占位。

### 综合决策与预测（健康与繁殖 → 产犊）
- 7 项特征按牛对齐到整点，每列派生 1 h / 6 h 均值、相对本牛 24 h 中位数变化、72 h 稳健 z 分数、6 h 斜率、日节律残差；基线滞后 6 h，避免产程污染基线。
- 产犊结局合并牧场登记与视频“犊牛完全娩出”标签（12 h 内以视频为准；24 次视频校准显示登记平均晚约 0.2 h）；只标注产前 10 天，产后与远离产犊的时段不当负例。
- 决策算法库 9 种：专家规则打分、本牛基线偏离度、L2 逻辑回归、决策树、随机森林、极端随机树、AdaBoost、XGBoost、堆叠集成（LR+RF+XGB→LR + 等渗校准）；另有分位数梯度提升给出距产犊 P10/P50/P90，并用共形校正（CQR）保证 80% 区间覆盖。全部模型以数值 JSON 保存，推理不加载 pickle。
- 验证：按牛分组交叉验证；交叉拟合等渗校准；阈值取 Youden 指数；报告 ROC/PR AUC（含按牛自助 95% CI）、Brier、ECE、灵敏度、特异度、MCC 等窗口指标，以及产犊检出率、提前量、误报/牛·天等事件指标；只在各特征自身计算时段内训练，覆盖率不作模型输入，防止“缺失即泄漏”。
- 输出：6 / 12 / 24 / 48 h 产犊校准概率（单调）、预警等级（正常 / 关注 / 高度关注 / 临产 / 数据不足）与建议、预计距产犊时间区间、TreeSHAP 主要驱动特征、特征覆盖与缺失、连续预警时段。
- 新窗口 5 个页签：滚动预测（风险趋势、预计产犊时间、预警时段、数据覆盖）、特征模块、决策数据集（产前规律曲线、单特征区分度）、决策训练（排行榜 + ROC / PR / 校准 / 风险-距产犊时间 / XGBoost 逐轮损失 / 学习曲线 / 特征组置换重要性 / 提前量分布 / 逐次产犊）、输出与评价指标（含去掉的指标与原因）；可勾选特征做消融对比。
- 真实数据（扬大高邮牧场）结果见 `docs/DECISION_RESULTS_433.md`。

### 7 项决策特征（统计数据规律与决策预测各会话交付）
- 温度 temperature-2：尾环温度按 10 min 输出在体中位数、最大值、站姿中位数与在体占比；产前 24 h 较前一天下降中位 0.37 °C，产前 6 h 急降约 1 °C；是综合决策中最强的特征。
- 活动量 activity-2：VeDBA、活动/高强度占比、活动回合、尾部姿态变化等，跨包合并；产前 6 h 内较本牛基线升高 43%–86%。
- 躺卧占比 lying-occupancy-2：姿态 + 起立/卧倒同步 HMM，加入脱环检测，10 min 误差 22.5→1.7 pp。
- 努责占比 straining-ratio-2：只计收缩脉冲链覆盖秒并做识别误差校正，与视频标签 CCC 0.80（1 h 0.83），远离产犊误检窗口降为原来的 1/5。
- 角速度频谱熵 gse-1：三轴角速度 Welch 谱求和的 0.1–5 Hz 归一化谱熵、频带占比与转动功率；属晚期信号（产前约 3 h）。
- 血氧 spo2-ratio-gated-1：红光/红外比值法 SpO₂、R 比值与灌注指数，逐窗质量门控；产前血氧无变化，灌注指数自产前约 4 天升高约 1.6 倍（绝对 SpO₂ 未经牛血气标定，只作本牛相对比较）。
- 心率 heart-rate-decision-1.0.0（见下节）。

### 行为识别 · 训练可视化
- 训练与识别窗口新增“阈值-召回曲线”“阈值-候选密度曲线”“各折召回”（每折模型只评其验证记录）与“输出与评价指标”页签；精确率 / F1 / 误报在背景未完整审核时保持为空并说明原因。

### 健康与繁殖 · 产犊预测 · 心率
- 新增尾根 PPG 心率算法 `algorithms/heart_rate_signal.py`（`ppg-hr-1.0.0`）：带通 + 分块伪迹剔除 + 谐波和谱，再由收缩峰 IBI、自相关、红光三路独立校验，给出 HIGH/MEDIUM/LOW/REJECT。不合格记录不输出心率。
- 新增因果心率决策特征 `algorithms/heart_rate.py`（`heart-rate-decision-1.0.0`），特征包括：
  - 本牛 24–96 h 前基线，去掉 ±4 bpm 日节律；
  - 心率上升量、稳健 z 分数、近 6 h 离散度、24 h 趋势；
  - 合格脉搏波比例及其下降、RMSSD 比值；
  - 证据等级 ELEVATED/STRONG。
- 采集时刻改用设备 `time` 字段：work_mode 6 固件在采集后约 62 min 才上传。决策只使用已上传的采集。
- 接入综合决策引擎插件 `cowmata_engine/features/heart_rate.py`（主列 `hr_level_6h_bpm`）。“生成综合证据 / 文件夹滚动预警”中的 `heart_rate_bpm` 不再恒为空。
- 扬大_高邮牧场实测（43 578 次采集，304 头）：
  - 合格率 62%，心率中位数 82.6 bpm；
  - 较本牛产前 7–4 天，产前 72 h 起约 +3 bpm，最后 24 h 中位 +6 bpm（82% 的牛上升）；
  - 仅心率特征、按牛验证的 AUC：6 h 0.70，24 h 0.67。
- 算法说明：`docs/heart-rate-algorithm.md`；数据集与报告：`4.3.3/心率/`。

## 4.3.2 · 2026-09-26

### 立即清盘
- 修复 4.3.1 点击“立即清盘…”总是提示“当前选择的原盘不是刚刚完成归类的那块盘，已取消清盘”：按钮的 clicked(bool) 信号把 False 当作“待核对的盘身份”传入，误走了归类后核对分支。现在按钮只走“勘查 → 一次确认 → 清盘”，只有归类完成后的清盘询问才核对磁盘身份。
- 新增真实按钮点击回归测试（在 4.3.1 代码上可复现该错误）。
- 已用修复后的清盘逻辑现场清空磁盘 3（HGST HUS726040ALE610 · 序列号 1C1A5C854321 · 4.00 TB）：4 个分区、68.8 MB 索引区 1.4 秒完成，复核剩余录像索引 0 条，磁盘格式与身份不变，G: 分区未改动。

### 行为识别 · 训练与识别
- 修复点击“行为识别 → 训练与识别…”弹出“COWMATA 运行错误”：算法窗口迁到 ui/algorithms 后，数据集默认目录的延迟导入仍指向不存在的 ui.algorithms.paths（ModuleNotFoundError），改为从 cowmata_tailring.algorithms.paths 读取。
- 全包静态检查全部内部导入，仅此一处失效；逐项点击“行为识别 / 健康与繁殖 / 数据集构建”全部 32 个菜单项均无异常。
- 修正两处仍指向旧模块路径的算法管理测试，使其真正覆盖窗口加载与关闭时取消训练。
## 4.3.1 · 2026-09-26

### 录像机原盘清盘
- 修复清盘报错“[Errno 87] 写入失败：分区0 描述符表”：原盘物理扇区为 4096 字节，描述符表长度（条目数×32）不是扇区整数倍，Windows 拒绝非对齐写入。现在所有清零都按 4 KiB 对齐，首尾不完整的块先读出、只改目标区间再整块写回；只卸载与清零区间重叠的卷（G: 等录像导出卷不受影响）。
- 流程一步完成：归类完成后先弹出“数据归类完成”，再询问“是否清盘”，点“立即清盘”即执行，完成或失败都有弹窗说明；“刷新原盘”旁的“立即清盘…”走同一流程（勘查→一次确认→清盘）。归类后清盘会核对磁盘身份，换了盘则自动取消。
- 4.3.0 的失败尝试可能已清空部分分区索引；上次归类已全部完成（已归档文件不受影响），在 4.3.1 中再点一次“立即清盘…”即可完成。

### 归类全程高速
- 定位中后段掉速：转换本身一直很快，但每条录像要在单一串行“提交”环节里完成 D:→F: 拷贝（约 1.5 GB/条）并重写 6.5 MB 的资源索引，40%~50% 以后队列越积越长，录像在“核验”状态排队 75~155 秒。
- 拷贝移到独立的暂存拷贝通道，在转换线程里与后续转换重叠执行；提交环节只做同盘改名。实测 F: 盘单路拷贝 185 MiB/s，2 路 132、4 路 109，因此固定 1 路顺序拷贝，避免 RAID5 多路抢写。
- 资源索引最多每 3 秒落盘一次（原来每条都重写，每次 0.16 s 以上），任务结束、取消或出错时一定写入最终完整索引。

### 标注：看清牛身记号
- “波形”布局新增“放大视频”（双击录像标题或 Ctrl+E）：视频铺满窗口宽度，下方保留波形条，可继续锁定特征区间；配合 F11 即全屏。
- 新增“波形分屏”（Ctrl+Shift+E）：波形移到独立窗口，自动放到第二块屏幕并最大化，主窗口整块显示视频；关闭该窗口即合并回来。分屏窗口里空格、[ ]、←→、标签快捷键照常可用。
- 视频原生画面不换父窗口、不重新加载，播放时钟和位置保持不变。

### 端侧数据下载
- 筛选只保留“全部 / 待下载 / 已下载”三项；部分已下载、今日数据、不下载等状态直接看表格首列颜色和文字。
- “更多”菜单收为 4 项：运行记录、问题清单与备注、台账与连接、打开目录。

### 菜单精简
- 保留 7 个主菜单（按工作流：文件 → 数据准备 → 标注与复核 → 数据集构建 → 行为识别 → 健康与繁殖 → 帮助），菜单最多两级，不重复。
- 数据准备：端侧数据下载、数据归类、导出标准 MP4 副本、录像索引与核验（原来藏在“更多数据准备工具 → 录像索引”第三级）。
- 标注与复核：15 项收为 候选 / 完成 / 撤销重做 + 标签编辑、时间同步、多人协作、显示与播放四个分组；“打开协作原始数据包”并入多人协作。
- 删除重复入口“算法管理”（与“行为识别 → 训练与识别”是同一窗口）；“逐项算法检查”移到“行为识别”。
- 显示与播放把放大视频、波形分屏、全屏放在最前。所有功能、快捷键和处理逻辑均保留，只调整位置。
## 4.2.8 - 2026-09-25

- 统一行为算法库：起立、卧倒、努责、排尿、胎儿首见、犊牛娩出均登记为训练/预测/候选审核能力，模型与运行结果外置到配置的模型根目录。
- 增加行为算法统一预测适配器与应用层路由；候选始终保持待人工确认，不写入人工标签。
- 大华原盘主路径采用顺序扫盘、16 路转换并行，并在运行报告中记录 parallel_workers 与 read_strategy。
- DOCX 上传继续使用 ZIP 结构校验、源文件稳定性检查和原子暂存，避免半写入和重名覆盖。
- 代码按算法、应用服务、UI 算法窗口分层，运行路径支持环境变量和持久化设置，源代码不含开发机绝对路径。
...
### 行为算法与架构
- 将起立、卧倒、努责、排尿、胎儿首个部位首次可见、犊牛完全娩出六类行为纳入统一能力矩阵，训练、预测和自动候选标注共用版本化套件与哈希合同。
- 新增可取消的候选扫描服务；结果只进入待人工复核队列，不把候选当作标签、概率或阴性样本。
- 将算法、工作区服务和 Qt 界面按三层边界整理；模型、缓存、曲线、OOF 和运行结果统一由 COWMATA_ALGORITHM_HOME 管理，源码不再包含开发机绝对路径。
- 产犊证据与决策输出继续保留模型版本、套件哈希和数据集指纹，健康结论不由行为事件自动冒充。

### 交付与验证
- 便携包、源码包、安装包均为 4.2.8；便携离线自检和真实启动自检通过。
- 源码包与便携包不含研究目录缓存、模型副产物或 .bak 文件；Release 只上传安装包。

## 4.2.7 · 2026-09-25

### 行为算法 · 胎儿首个部位首次可见（FETAL_PART_FIRST_VISIBLE）
- 新增标准算法 `cowmata_tailring/algorithms/fpfv.py`（特征 `fpfv_features.py`，版本 `fpfv-tail-hold-3`）。它按牛把前后记录按 epoch 拼接成连续序列，用两套特征：尾抬角 `elev=arccos(−u_y)`（不受尾环转动影响），以及以本牛 24 h 尾姿为基线的相对特征。模型先估计第二产程状态概率，再取上升沿作为 FPFV 近似时刻，每 12 h 最多一个，全部需视频确认。
- 模型为数值 JSON 树（`numeric-gbdt-1`），推理不加载 pickle，只依赖 numpy/pandas。验证方法与结果见 `docs/FPFV_ALGORITHM.md`：按牛留一、连续流评估，与第一版方法在同一协议下对照。
...
- 推算：约 753 GB 原始数据按企业级机械盘顺序读 ~150 MiB/s 计约 1.4 小时读盘，16 路转换并行重叠执行，3274 段总时长预计 **2-3 小时**（10 小时目标内，余量充足）。
- 排查确认工程索引 0 条错误：此前"未索引"是排队等待（每段需 SHA-256 校验+帧时钟扫描数秒），4.2.0 的 3 路并行后台索引已在消化队列；标注窗口内优先索引当前窗口录像的调度保持不变。
- 录像机时钟回跳自动恢复：PTS 回跳/重启（实测 -0.84s/+0.92s 模式）的原始段不再进入“待核对”，而是重建单调帧槽时间轴后照常转码归档；超出恢复范围（单次回退 >5 秒）的少量损伤段（实测 3/3274 ≈ 0.1%）仍保留人工复核通道。
- 音频流损坏自动降级：pcm_alaw/aresample/非单调 DTS 等音频侧失败时，自动丢弃损坏音频、完整保留视频归档（成品标注“音频已丢弃”），不再阻塞整段归类。

## 4.2.0 · 2026-09-22

- 原始录像归类提升为 **16 路并行**（每个已映射视角一路，与常规目录模式一致）：DHFS 磁盘按时间交错物理排布，公平轮询读盘天然接近顺序；同时保留 6 深度的原盘读盘队列保护磁头，其余视角在后台转码/校验/归档。3274 段任务预计 **2-4 小时**完成（远低于 10 小时目标），且对任意接入的录像机原盘生效。
- 后台完整索引升级为 **3 路并行检查**（每线程独立 SourceInspector，OCR 模型不跨线程共享；目录提交仍然串行保护）。标注窗口内保持原有单路顺序，保证标注完成的先后次序稳定。
- 沿用 4.1.9：更新器自动清理安装目录内 `%SystemDrive%` 缓存副本与 `NVIDIA umdlogs` 遗留物；“继续上次归类”自动跳过来源原盘已断开的旧任务；数据归类窗口显示几何自动钳制回屏幕。

## 4.1.9 · 2026-09-22

- 修复“保存退出并更新”被安装目录内的陌生文件卡住的问题：Windows 组件与显卡安装器偶尔会在软件目录里落下一个字面量 `%SystemDrive%` 缓存副本或 `NVIDIA Corporation/umdlogs` 日志，更新器现在会在目录校验前自动清掉这两类已知的安装器遗留物（其余陌生文件仍照常拦截保护）。
- 修复“继续上次归类”在牧场同时存在新旧两个未完成任务时报“多个未完成的视频任务”的问题：来源录像机原盘已断开的旧任务自动跳过，直接恢复当前可继续的任务；全部断开时提示接盘。
- 修复数据归类窗口重新打开时标题栏和原盘选择行被屏幕外遮挡的问题：窗口每次显示都会钳制回当前屏幕范围内。
- 已在本机完成对应实测：安装目录垃圾已清理（更新校验恢复通过）、双任务牧场恢复正确选中当前 4TB 原盘任务、归类窗口几何钳制生效。

## 4.1.8 · 2026-09-21

- 归类提速：录像机原盘模式下新增“读盘—转换”流水线，读盘保持严格单路顺序（保护机械盘磁头），转换/校验与下一盘段读取完全重叠；模拟实验实测同类工作量加速约 2.1 倍。3274 段任务按实测 14.2 MiB/秒推算约 14 小时，提速后预计 6-7 小时内完成（10 小时目标内）。
- 更新通道重构：自动更新改用 6 连接分段下载 + 断点续传 + 卡流自动重连（原为单连接且仅 3 次重试，国内线路易卡死），下载不再反复卡住，SHA-256 完整性校验保持不变。
- 新增“立即清盘”：在原始录像归类页选择录像机原盘后，可立即将该盘恢复为空白 DHFS4.1 格式（保留原盘头、分区几何与超级块，仅清空录像索引与数据首块），清盘后自动复核为 0 条录像。需两次确认（含输入“清盘”），系统盘与正在归类的磁盘会被拒绝。
- 修复主视角原生播放渲染错位（4.1.7 已含）、画中画拖动与波形完整记录切换（4.1.7 已含）随本版一并发布。

## 4.1.7 · 2026-09-21

- 修复带 `.mp4` 文件名的大华 MPEG-PS 原始码流按相邻文件名猜时长后，中段画面“无法解码所选位置的真实画面/精确帧读取失败”的问题：现在优先用录像机自身帧时钟和字节级关键帧索引建立时间轴（真实时长、可精确定位），相邻文件名只作为扫描失败时的浏览兜底。
- 已按旧规则索引过的此类录像会在打开工程时自动重建一次（人工标注、校准和对齐保留），多个日期目录的同类文件一并修复。
- 无九轴记录直接浏览视频时，按需索引现在跟随播放头所选视角工作；此前“打开工程后本视角仍有录像未索引”会一直等待，只有选中九轴记录或手动完整索引才开始。
- 修复“继续上次归类”恢复暂停任务时，因新版恢复请求比旧任务多出运行时限字段（deadline_seconds）而必然报“恢复任务的范围或映射已变化；请重新扫描建立新任务”的问题：恢复时回放任务保存的原始请求，且签名校验容忍版本新增的默认字段；来源、范围或映射的真实变化仍会照常拦截。
- 修复主视角原生播放画面渲染成窄条/错位子矩形的问题：布局尺寸变化后自动按当前画面尺寸重建视频输出，解码与时间轴不受影响；此问题同时表现为“主视角画面卡住不动”“开始播放成功但没有视频画面”。
- 波形页画中画标题栏增加鼠标抓取，快速拖动不再断开；波形“更多”菜单选择“完整记录”会先退出平移模式再复位视图。
- 以上修复仅限缺陷相关逻辑，其他功能保持不变。

## 4.1.5 · 2026-09-21

- 修复同一视角存在相同起始时间的 `xxx.mp4` 与 `xxx__001.mp4` 时，按需索引漏掉重复时间桶，导致后续长录像持续 pending、视角 11–16 无法播放的问题。
- 保留重复源文件身份和独立校验，不合并、不覆盖、不虚构录像覆盖范围。
- 修复本机 4.1.4 更新后软件代码与安装清单/身份元数据错位的收尾验证路径。

## 4.1.4 · 2026-09-21

- 原始 Dahua MPEG-PS 录像保留原文件名时间并用相邻片段建立仅浏览时间范围，避免因错误的超长容器头时长而长时间停在“未索引”；该范围仍标记待复核，不冒充已校准证据。
- 物理录像机原盘改为串行读取，避免多路随机寻道导致读取阶段吞吐骤降；普通文件来源仍保留多视角并行。
- 转码运行汇总只按已完成段计算平均速度，不把尚在读取原盘的等待时间混入已完成吞吐；“原始录像转码与归类”继续作为数据归类窗口首个页签。
- 发布包升级校验要求源码、便携包和安装目录的关键界面文件一致。

## 4.1.0 · 2026-09-19

- 版本切换时清理由版本标记确认、且仅由工程维护清单拥有的派生播放缓存；原始录像、传感器数据、标注、证据和未完成任务保留。其他窗口占用工程时延后清理，释放后自动重试。
- 修复旧版本因便携包媒体工具未就绪而被标记为异常的录像；升级后只重新排队这类可恢复记录，其他解析错误继续保留并明确显示。
- 继续使用便携包内置 FFmpeg/FFprobe 完成视频解析，避免依赖机器全局安装；新增版本缓存与视频恢复回归测试。
- 导出 4.1.0 安装包、便携包、纯净源码包，并同步 GitHub 主分支与 Release 安装包。

## 4.0.0 · 2026-09-19

- 正式版沿用3.9.9已验证的数据目录、管理员/操作员工作区和可恢复任务协议。
- 软件正常退出后仅清理已登记工程中的空归类缓存目录，并把有效的资源索引备份移入应用恢复区；未完成任务、下载去重库、标注工程、媒体和传感器数据保持不变。
- 管理员工程打开时默认加载九轴、PPG、温度和录像，仍支持按页签查看单类数据。
- 更新安装包、便携包和源码包，安装包发布到GitHub Release。

# 3.9.5 — 2026-09-17

严格台账选择性下载、多目录后台归类、1/2/7/9/A 候选、历史证据与画中画修复、管理员与操作员登录权限，以及可搜索的完整新手图文教程。详见 [本版说明](docs/release-395.md)。
...
- 统一独立温度与历史 Motion 温度、数据集导出和温度模型输入契约。
- 下载器对齐牧场首帧时间戳命名，禁止冲突哈希后缀；损坏记录先备份再恢复。
- 校验温度单位、牛号与数据可用时刻，保留原始传感值和标注坐标。
- 详见 docs/release-393.md。

# 3.9.2 — 2026-09-16

- 恢复 GitHub 更新发布与 3.8+ 兼容升级，API 限流与 TLS 中断提供官方备用通道。
- 最新安装器可以原位升级旧便携版，保留完整校验、失败回滚与用户文件保护。
- 数据下载改为先选手动/自动/定时再启动；按原始时间戳命名，跨分类核对 Motion/PPG，已有记录不重复下载。
- CSV 监测目的和登录协议对齐上传器 1.3.1。
- 包含 3.9.1 标签快捷键、下载路径及训练路径修复。

# 3.9.0 — 2026-09-16

- 三 CSV 自动配置设备、耳标、现场标号、佩戴时段及下载归类；完整设备号保留，仅四位简写补全。
- 统一九轴、PPG、温度下载目录与标注、Raw/Label 数据集输入。
- 新增行为识别训练/识别工作台、外置模型库、训练历史与评价可视化。
- 产犊综合决策接入 XGBoost 3.4.1、随机森林和决策树，按牛分组验证并保留缺失指标。
- 修复按日期打开 PPG；CSV 状态和备份移入应用数据目录。
- 仅源码 ZIP、便携 ZIP 和解压目录；详细说明见 docs/release-390.md。

# 3.8.4

端侧数据下载接入独立下载器的自动分页与上传器 1.1.0 三 CSV 只读同步；服务器连接和本地/服务器保存位置可分别核对。详见 docs/release-384.md。

# 3.8.2 — 2026-09-15

## 3.8.3 — 2026-09-15

- 默认一次对齐即可确认标注，提供可选的漂移精细校准；保留旧校准及撤销。
- 校准表保存保留毫秒与原始画面依据。
- 增加盘符下一级安装目录的更新准备回归检查，说明旧 3.8.0 的兼容升级方式。

独立算法管理、起立/卧倒/努责候选、四项产犊证据与趋势；合入3.8.1完整数据集和更新修复。见[本版说明](docs/release-382.md)。
...
- 修复录像机 MPEG-PS / HEVC 文件按错误短时长建立索引导致视角01空白的问题。
- 视角01等旧录像改用逐帧时钟和关键帧索引，时间轴保留原目录与文件名。
- 增加完整视频核验与异常路径记录；打开工程可显示无法加载的具体原因。
- 优化工作区：隐藏重复品牌与打开工程按钮，压缩辅助控件，增大曲线区域，将九轴、PPG、温度页签移至曲线下方。
- 增加“关闭标注”按钮，保存并释放工程文件后再进行数据归类。
- 派包按选定日期保持牧场目录树，只派对应 JSON 与视频文件。
# 4.1.3

- 修复原始转码任务在已归档目标缺失时错误阻断的问题：原始 G 盘记录仍可读时自动重新转码，不清理原盘。
